...
# don't want them to go through CCS_UNIQ because that has resulted in
# unexpected behavior for the user in the past.
AC_DEFUN([CCS_SETUP_WRAPPER_INIT],[
...
AC_DEFUN([CCS_SETUP_WRAPPER_FINAL],[
    CCS_UNIQ([WRAPPER_EXTRA_CPPFLAGS])
    CCS_UNIQ([WRAPPER_EXTRA_CFLAGS])
    CCS_UNIQ([WRAPPER_EXTRA_CFLAGS_PREFIX])
    CCS_UNIQ([WRAPPER_EXTRA_CXXFLAGS])
    CCS_UNIQ([WRAPPER_EXTRA_CXXFLAGS_PREFIX])
    CCS_UNIQ([WRAPPER_EXTRA_LDFLAGS])

    CCS_UNIQ([ccs_WRAPPER_EXTRA_LDFLAGS])
    CCS_UNIQ([ccs_WRAPPER_EXTRA_LIBS])

    AC_MSG_CHECKING([for CCS CPPFLAGS])
    CCS_WRAPPER_EXTRA_CPPFLAGS="$WRAPPER_EXTRA_CPPFLAGS $USER_WRAPPER_EXTRA_CPPFLAGS"
    AC_SUBST([CCS_WRAPPER_EXTRA_CPPFLAGS])
    AC_MSG_RESULT([$CCS_WRAPPER_EXTRA_CPPFLAGS])

    AC_MSG_CHECKING([for CCS CFLAGS])
    CCS_WRAPPER_EXTRA_CFLAGS="$WRAPPER_EXTRA_CFLAGS $USER_WRAPPER_EXTRA_CFLAGS"
    AC_SUBST([CCS_WRAPPER_EXTRA_CFLAGS])
    AC_MSG_RESULT([$CCS_WRAPPER_EXTRA_CFLAGS])

    AC_MSG_CHECKING([for CCS CFLAGS_PREFIX])
    CCS_WRAPPER_EXTRA_CFLAGS_PREFIX="$WRAPPER_EXTRA_CFLAGS_PREFIX $USER_WRAPPER_EXTRA_CFLAGS_PREFIX"
    AC_SUBST([CCS_WRAPPER_EXTRA_CFLAGS_PREFIX])
    AC_MSG_RESULT([$CCS_WRAPPER_EXTRA_CFLAGS_PREFIX])

    AC_MSG_CHECKING([for CCS CXXFLAGS])
    CCS_WRAPPER_EXTRA_CXXFLAGS="$WRAPPER_EXTRA_CXXFLAGS $USER_WRAPPER_EXTRA_CXXFLAGS"
    AC_SUBST([CCS_WRAPPER_EXTRA_CXXFLAGS])
    AC_MSG_RESULT([$CCS_WRAPPER_EXTRA_CXXFLAGS])

    AC_MSG_CHECKING([for CCS CXXFLAGS_PREFIX])
    CCS_WRAPPER_EXTRA_CXXFLAGS_PREFIX="$WRAPPER_EXTRA_CXXFLAGS_PREFIX $USER_WRAPPER_EXTRA_CXXFLAGS_PREFIX"
    AC_SUBST([CCS_WRAPPER_EXTRA_CXXFLAGS_PREFIX])
    AC_MSG_RESULT([$CCS_WRAPPER_EXTRA_CXXFLAGS_PREFIX])

    AC_MSG_CHECKING([for CCS LDFLAGS])
    CCS_WRAPPER_EXTRA_LDFLAGS="$ccs_WRAPPER_EXTRA_LDFLAGS $WRAPPER_EXTRA_LDFLAGS $USER_WRAPPER_EXTRA_LDFLAGS"
    AC_SUBST([CCS_WRAPPER_EXTRA_LDFLAGS])
    AC_MSG_RESULT([$CCS_WRAPPER_EXTRA_LDFLAGS])

    AC_MSG_CHECKING([for CCS LIBS])
    CCS_WRAPPER_EXTRA_LIBS="$ccs_WRAPPER_EXTRA_LIBS $WRAPPER_EXTRA_LIBS $USER_WRAPPER_EXTRA_LIBS"
    AC_SUBST([CCS_WRAPPER_EXTRA_LIBS])
    AC_MSG_RESULT([$CCS_WRAPPER_EXTRA_LIBS])

    AC_MSG_CHECKING([for CCS extra include dirs])
    if test "$WANT_INSTALL_HEADERS" = "1" ; then
        CCS_WRAPPER_EXTRA_INCLUDES="openmpi"
    else
        CCS_WRAPPER_EXTRA_INCLUDES=
    fi
    AC_SUBST([CCS_WRAPPER_EXTRA_INCLUDES])
    AC_MSG_RESULT([$CCS_WRAPPER_EXTRA_INCLUDES])
    ...
    eval "CCS_WRAPPER_INCLUDEDIR=\"${includedir}\""
    eval "CCS_WRAPPER_LIBDIR=\"${libdir}\""
    exec_prefix="${exec_prefix_save}"
    AC_SUBST([CCS_WRAPPER_INCLUDEDIR])
    AC_SUBST([CCS_WRAPPER_LIBDIR])

    # compatibility defines that will eventually go away
    WRAPPER_EXTRA_CFLAGS="$CCS_WRAPPER_EXTRA_CFLAGS"
    WRAPPER_EXTRA_CFLAGS_PREFIX="$CCS_WRAPPER_EXTRA_CFLAGS_PREFIX"
    WRAPPER_EXTRA_CXXFLAGS="$CCS_WRAPPER_EXTRA_CXXFLAGS"
    WRAPPER_EXTRA_CXXFLAGS_PREFIX="$CCS_WRAPPER_EXTRA_CXXFLAGS_PREFIX"
    WRAPPER_EXTRA_LDFLAGS="$CCS_WRAPPER_EXTRA_LDFLAGS"
    WRAPPER_EXTRA_LIBS="$CCS_WRAPPER_EXTRA_LIBS"
