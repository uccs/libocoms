...
AC_DEFUN([CCS_CONFIGURE_OPTIONS],[
ccs_show_subtitle "CCS Configuration options"
...
if test "$WANT_MEM_DEBUG" = "0" -a -z "$enable_mem_debug" -a "$CCS_DEVEL" = 1; then
    WANT_MEM_DEBUG=1
    echo "--> developer override: enable mem profiling by default"
fi
#################### Early development override ####################
AC_DEFINE_UNQUOTED(CCS_ENABLE_MEM_DEBUG, $WANT_MEM_DEBUG,
...
#################### Early development override ####################
if test "$WANT_MEM_PROFILE" = "0" -a -z "$enable_mem_profile" -a "$CCS_DEVEL" = 1; then
    WANT_MEM_PROFILE=1
    echo "--> developer override: enable mem profiling by default"
fi
#################### Early development override ####################
AC_DEFINE_UNQUOTED(CCS_ENABLE_MEM_PROFILE, $WANT_MEM_PROFILE,
    [Whether we want the memory profiling or not])
...
#################### Early development override ####################
if test "$WANT_PICKY_COMPILER" = "0" -a -z "$enable_picky" -a "$CCS_DEVEL" = 1; then
    WANT_PICKY_COMPILER=1
...
#################### Early development override ####################
if test "$WANT_DEBUG" = "0" -a -z "$enable_debug" -a "$CCS_DEVEL" = 1; then
    WANT_DEBUG=1
...
AC_DEFINE_UNQUOTED(CCS_ENABLE_DEBUG, $WANT_DEBUG,
...
#
# Do we want to install all of CCS/ORTE and OMPI's header files?
#
...
AC_DEFINE_UNQUOTED([CCS_WANT_PRETTY_PRINT_STACKTRACE],
...
if test "$enable_pty_support" = "no" ; then
    AC_MSG_RESULT([no])
    CCS_ENABLE_PTY_SUPPORT=0
else
    AC_MSG_RESULT([yes])
    CCS_ENABLE_PTY_SUPPORT=1
fi
AC_DEFINE_UNQUOTED([CCS_ENABLE_PTY_SUPPORT], [$CCS_ENABLE_PTY_SUPPORT],
...
if test "$enable_dlopen" = "no" ; then
    ...
    CCS_ENABLE_DLOPEN_SUPPORT=0
    AC_MSG_RESULT([no])
else
    CCS_ENABLE_DLOPEN_SUPPORT=1
    AC_MSG_RESULT([yes])
fi
...
if test "$enable_heterogeneous" = "yes" ; then
     AC_MSG_RESULT([yes])
     ccs_want_heterogeneous=1
else
     AC_MSG_RESULT([no])
     ccs_want_heterogeneous=0
fi
AC_DEFINE_UNQUOTED([CCS_ENABLE_HETEROGENEOUS_SUPPORT], 
                   [$ccs_want_heterogeneous], 
...
AC_ARG_ENABLE([trace],
    [AC_HELP_STRING([--enable-trace],
                    [Enable internal tracing of OMPI/ORTE/CCS calls -- used only for developer debugging, not tracing of MPI applications (default: disabled)])])
...
AC_DEFINE_UNQUOTED([CCS_ENABLE_TRACE], [$ccs_want_trace],
...
if test "$with_cross" = "yes" ; then
    ...
    CCS_LOG_MSG([Loading cross-compile file $with_cross, with contents below])
    CCS_LOG_FILE([$with_cross])
    . "$with_cross"
fi
...
#    - orcm
#  /* General FT sections */
#  #if CCS_ENABLE_FT == 0 /* FT Disabled globaly */
#  #if CCS_ENABLE_FT == 1 /* FT Enabled globaly */
#  /* CR Specific sections */
#  #if CCS_ENABLE_FT_CR == 0 /* FT Ckpt/Restart Disabled */
#  #if CCS_ENABLE_FT_CR == 1 /* FT Ckpt/Restart Enabled */
#
...
AC_ARG_WITH(ft,
    [AC_HELP_STRING([--with-ft=TYPE],
        ...
        [ccs_want_ft=1],
        [ccs_want_ft=0])
if test "$with_ft" = "no" -o "$ccs_want_ft" = "0"; then
    ccs_want_ft=0
    ccs_want_ft_cr=0
    ...
    if test "$ccs_want_ft_cr" = 1; then
        ccs_want_ft_type="cr"
    elif test "$ccs_want_ft_orcm" = 1; then
        ccs_want_ft_type="orcm"
    fi

    AC_MSG_RESULT([Enabled $ccs_want_ft_type (Specified $with_ft)])
    AC_MSG_WARN([**************************************************])
...
AC_DEFINE_UNQUOTED([CCS_ENABLE_FT], [$ccs_want_ft],
                   [Enable fault tolerance general components and logic])
AC_DEFINE_UNQUOTED([CCS_ENABLE_FT_CR], [$ccs_want_ft_cr],
                   [Enable fault tolerance checkpoint/restart components and logic])
AM_CONDITIONAL(WANT_FT, test "$ccs_want_ft" = "1")
...
AM_CONDITIONAL([CCS_INSTALL_BINARIES], [test "$enable_binaries" != "no"])
...
AM_CONDITIONAL([CCS_WANT_SCRIPT_WRAPPER_COMPILERS], [test "$enable_script_wrapper_compilers" = "yes"])
...
AC_DEFINE_UNQUOTED([CCS_WANT_HOME_CONFIG_FILES], [$result],
...
AC_DEFINE_UNQUOTED([CCS_ENABLE_IPV6], [$ccs_want_ipv6],
...
if test "$with_package_string" = "" -o "$with_package_string" = "no"; then
    with_package_string="Open MPI $CCS_CONFIGURE_USER@$CCS_CONFIGURE_HOST Distribution"
fi
AC_DEFINE_UNQUOTED([CCS_PACKAGE_STRING], ["$with_package_string"],
...
with_ident_string="`echo $with_ident_string | sed -e 's/%VERSION%/'$CCS_VERSION/`"
AC_DEFINE_UNQUOTED([CCS_IDENT_STRING], ["$with_ident_string"],
...
AC_DEFINE_UNQUOTED([CCS_ENABLE_CRDEBUG], [$ccs_want_prd],
...
AS_IF([test "$ccs_check_cuda_happy" = "yes"],
...
if test "$ccs_check_cuda_happy" = "yes"; then
    AC_MSG_RESULT([yes (-I$with_cuda/include -L$with_cuda_libdir -lcuda)])
    CUDA_SUPPORT=1
    ccs_datatype_CPPFLAGS="-I$with_cuda/include"
    ccs_datatype_LIBS="-L$with_cuda_libdir -lcuda"
    AC_SUBST([ccs_datatype_CPPFLAGS])
    AC_SUBST([ccs_datatype_LIBS])
else
    AC_MSG_RESULT([no])
    CUDA_SUPPORT=0
fi

AM_CONDITIONAL([CCS_cuda_support], [test "x$CUDA_SUPPORT" = "x1"])
AC_DEFINE_UNQUOTED([CCS_CUDA_SUPPORT],$CUDA_SUPPORT,
