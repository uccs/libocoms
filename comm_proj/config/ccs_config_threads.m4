...
# create templates
AH_TEMPLATE([CCS_HAVE_SOLARIS_THREADS], 
    [Do we have native Solaris threads])
AH_TEMPLATE([CCS_HAVE_POSIX_THREADS], 
    [Do we have POSIX threads])

#
# Check for thread types - add your type here...
#
CCS_CONFIG_POSIX_THREADS(HAVE_POSIX_THREADS=1, HAVE_POSIX_THREADS=0)
...
CCS_CONFIG_SOLARIS_THREADS(HAVE_SOLARIS_THREADS=1, HAVE_SOLARIS_THREADS=0)
...
#
AH_TEMPLATE([CCS_THREADS_HAVE_DIFFERENT_PIDS],
    [Do threads have different pids (pthreads on linux)])

if test "$THREAD_TYPE" = "solaris"; then
    AC_DEFINE(CCS_HAVE_SOLARIS_THREADS, 1)
    AC_DEFINE(CCS_HAVE_POSIX_THREADS, 0)
    AC_DEFINE(CCS_THREADS_HAVE_DIFFERENT_PIDS, 0)
    ...
    AC_DEFINE(CCS_HAVE_SOLARIS_THREADS, 0)
    AC_DEFINE(CCS_HAVE_POSIX_THREADS, 1)
...
CCS was unable to find threading support on your system.  The
CCS development team is considering requiring threading support for
proper CCS execution.  This is in part because we are not aware of
...
AM_CONDITIONAL(CCS_HAVE_POSIX_THREADS, test "$THREAD_TYPE" = "posix")
AM_CONDITIONAL(CCS_HAVE_SOLARIS_THREADS, test "$THREAD_TYPE" = "solaris")

#
# Now configure the whole CCS and progress thread gorp
#
AC_MSG_CHECKING([if want CCS thread support])
AC_ARG_ENABLE([ccs-multi-threads],
    AC_HELP_STRING([--enable-ccs-multi-threads],
        [Enable thread support inside CCS (default: disabled)]),
    [enable_ccs_multi_threads="$enableval"],
    [enable_ccs_multi_threads="undef"])

if test "$enable_ccs_multi_threads" = "undef" ; then 
dnl    # no argument given either way.  Default to whether
dnl    # we have threads or not
dnl    if test "$THREAD_TYPE" != "none" ; then
dnl        CCS_ENABLE_MULTI_THREADS=1
dnl        enable_ccs_multi_threads="yes"
dnl    else
dnl        CCS_ENABLE_MULTI_THREADS=0
dnl        enable_ccs_multi_threads="no"
dnl    fi
    # no argument - default to no, but leave
    # enable_ccs_multi_threads="undef" so we
    # can later detect that it wasn't specified
    CCS_ENABLE_MULTI_THREADS=0
elif test "$enable_ccs_multi_threads" = "no" ; then
    CCS_ENABLE_MULTI_THREADS=0
else
    # they want CCS thread support.  Make sure we have threads
    if test "$THREAD_TYPE" != "none" ; then
...
AC_MSG_RESULT([$enable_ccs_multi_threads])
...
#
# --enable-ft-thread
#  #if CCS_ENABLE_FT_THREAD == 0 /* Disabled */
#  #if CCS_ENABLE_FT_THREAD == 1 /* Enabled  */
# 
AC_MSG_CHECKING([if want fault tolerance thread])
AC_ARG_ENABLE([ft_thread],
    [AC_HELP_STRING([--disable-ft-thread],
                    [Disable fault tolerance thread running inside all processes. Requires CCS thread support (default: enabled)])],
...
elif test "$enable_ft_thread" = "undef" -a "$enable_ccs_multi_threads" = "no" ; then
    ccs_want_ft_thread=0
    AC_MSG_RESULT([Disabled (CCS Thread Support Disabled)])
# if default, and MPI threads enabled
else
    # Default: Enable
    # Make sure we have CCS Threads enabled 
    if test "$enable_ccs_multi_threads" = "no"; then
        AC_MSG_RESULT([Must enable CCS basic thread support to use this option])
        AC_MSG_ERROR([Cannot continue])
    else
        AC_MSG_RESULT([yes])
        ccs_want_ft_thread=1
        AC_MSG_WARN([**************************************************])
        AC_MSG_WARN([*** Fault Tolerance with a thread in Open MPI    *])
        AC_MSG_WARN([*** is an experimental, research quality option. *])
        AC_MSG_WARN([*** It requires CCS thread support and care     *])
        AC_MSG_WARN([*** should be used when enabling these options.  *])
        AC_MSG_WARN([**************************************************])
    fi
fi
AC_DEFINE_UNQUOTED([CCS_ENABLE_FT_THREAD], [$ccs_want_ft_thread],
                   [Enable fault tolerance thread in Open PAL])
AM_CONDITIONAL(WANT_FT_THREAD, test "$ccs_want_ft_thread" = "1")
