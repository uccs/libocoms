...
AC_DEFUN([CCS_SETUP_LIBLTDL],[
    CCS_VAR_SCOPE_PUSH([HAPPY])

    ccs_show_subtitle "GNU libltdl setup"
    ...
    LIBLTDL_SUBDIR=
    CCS_HAVE_LTDL_ADVISE=0
    CCS_LIBLTDL_INTERNAL=0

    AS_IF([test "$CCS_ENABLE_DLOPEN_SUPPORT" = "0"],
           ...
           *) 
              ...
              CCS_CHECK_WITHDIR([libltdl], [$libltdl_location], 
                                 [include/ltdl.h])
              # If we're using an extern libltdl, then reset the
              # LTDLINCL that was set earlier (ie., there's no need to
              # -I into our internal libltdl tree).
    ...
    AC_DEFINE_UNQUOTED(CCS_HAVE_LTDL_ADVISE, $CCS_HAVE_LTDL_ADVISE,
        [Whether libltdl appears to have the lt_dladvise interface])

    AC_DEFINE_UNQUOTED(CCS_WANT_LIBLTDL, $CCS_ENABLE_DLOPEN_SUPPORT,
        [Whether to include support for libltdl or not])
    AC_DEFINE_UNQUOTED(CCS_LIBLTDL_INTERNAL, $CCS_LIBLTDL_INTERNAL,
        [Whether we are using the internal libltdl or not])

    AM_CONDITIONAL(CCS_HAVE_DLOPEN, 
                   [test "$CCS_ENABLE_DLOPEN_SUPPORT" = "1"])
    CCS_VAR_SCOPE_POP([HAPPY])
])dnl
...
#
AC_DEFUN([_CCS_SETUP_LIBLTDL_INTERNAL],[
    ccs_subdir_args="$ccs_subdir_args --enable-ltdl-convenience --disable-ltdl-install"
    if test "$enable_shared" = "yes"; then
        ccs_subdir_args="$ccs_subdir_args --enable-shared"
    else
        ccs_subdir_args="$ccs_subdir_args --disable-shared"
    fi
    if test "$enable_static" = "yes"; then
        ccs_subdir_args="$ccs_subdir_args --enable-static"
    else
        ccs_subdir_args="$ccs_subdir_args --disable-static"
    fi

    CFLAGS_save="$CFLAGS"
    CFLAGS="$CCS_CFLAGS_BEFORE_PICKY $CCS_VISIBILITY_CFLAGS"

    CCS_CONFIG_SUBDIR(service/libltdl, [$ccs_subdir_args], 
                       [HAPPY=1], [HAPPY=0])
    if test "$HAPPY" = "1"; then
        LIBLTDL_SUBDIR=libltdl
        CCS_LIBLTDL_INTERNAL=1

        CPPFLAGS_save="$CPPFLAGS"
        CPPFLAGS="-I."
        AC_EGREP_HEADER([lt_dladvise_init], [ccs/libltdl/ltdl.h],
                        [CCS_HAVE_LTDL_ADVISE=1])
        CPPFLAGS="$CPPFLAGS_save"

        # Arrgh.  This is gross.  But I can't think of any other
        # way to do it.  :-(
        flags=`$EGREP ^LIBADD_DL ccs/libltdl/Makefile | cut -d= -f2-`
        CCS_CHECK_LINKER_FLAGS([ccs/libltdl/libtool], 
        ...
        AC_MSG_WARN([ccs/libltdl/config.log.  You can also disable GNU libltdl, which will disable])
