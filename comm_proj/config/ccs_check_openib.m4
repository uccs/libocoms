...
# CCS_CHECK_OPENIB(prefix, [action-if-found], [action-if-not-found])
# --------------------------------------------------------
# check if OPENIB support can be found.  sets prefix_{CPPFLAGS, 
# LDFLAGS, LIBS} as needed and runs action-if-found if there is
# support, otherwise executes action-if-not-found
AC_DEFUN([CCS_CHECK_OPENIB],[
    CCS_VAR_SCOPE_PUSH([$1_msg])
    ...
    CCS_CHECK_WITHDIR([openib], [$with_openib], [include/infiniband/verbs.h])
    AC_ARG_WITH([openib-libdir],
       [AC_HELP_STRING([--with-openib-libdir=DIR],
             [Search for OpenFabrics libraries in DIR])])
    CCS_CHECK_WITHDIR([openib-libdir], [$with_openib_libdir], [libibverbs.*])
...
dnl           fi
           ...
           if test "1" = "$$1_have_rdmaoe"; then
                AC_DEFINE_UNQUOTED([CCS_HAVE_RDMAOE], [$$1_have_rdmaoe], [Enable RDMAoE support])
                AC_MSG_RESULT([yes])
           else
                AC_MSG_RESULT([no])
           fi
    ...
    AC_DEFINE_UNQUOTED([CCS_HAVE_CONNECTX_XRC], [$$1_have_xrc],
    ...
    AC_DEFINE_UNQUOTED([CCS_HAVE_RDMACM], [$$1_have_rdmacm],
    ...
    AC_DEFINE_UNQUOTED([CCS_HAVE_IBCM], [$$1_have_ibcm],
