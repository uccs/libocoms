...
#rlg debug
    print("\n FFF in process project pname :: $pname  pdir :: $pdir :: \n");
#end rlg debug
...
#rlg debug
        print("\n \n*** Found ARG $p->{name} project :: dir $p->{dir} \n");
#end rlg debug
        if (-d "$topdir/$p->{dir}") {
            verbose "\n*** Found $p->{name} project :: dir $p->{dir} \n";
...
# rlg debug
print("\n FFF 1 pname - $pname ; $pdir --  dir: glob $mca_found->{$pname} \n");
printf("\n FFF after dump \n");
# end rlg
        if (exists($mca_found->{$pname})) {
            my $frameworks_comma;
# rlg debug
print("\n FFF 1 pname - $pname ; $pdir -- exists \n");
# end rlg
...
# debug
                        print("\n TTT found configure.m4 for - $pdir - $f - $c \n");
# end debug
                    } else {
