...
typedef char* ccs_iov_base_ptr_t;
#else
typedef void* ccs_iov_base_ptr_t;
#endif
 ...
 */
static inline uint64_t ccs_ptr_ptol( void* ptr ) __service_attribute_const__;
static inline uint64_t ccs_ptr_ptol( void* ptr )
{
    return (uint64_t)(uintptr_t) ptr;
}

static inline void* ccs_ptr_ltop( uint64_t value ) __service_attribute_const__;
static inline void* ccs_ptr_ltop( uint64_t value )
{
#if SIZEOF_VOID_P == 4 && SERVICE_ENABLE_DEBUG
    if (value > ((1ULL << 32) - 1ULL)) {
        opal_output(0, "Warning: truncating value in ccs_ptr_ltop");
    }
#endif
...
static inline uint16_t opal_swap_bytes2(uint16_t val) __service_attribute_const__;
...
static inline uint32_t opal_swap_bytes4(uint32_t val) __service_attribute_const__;
...
static inline uint64_t opal_swap_bytes8(uint64_t val) __service_attribute_const__;
