...
AC_DEFUN([CCS_SETUP_CC],[
    ...
    AC_REQUIRE([_CCS_START_SETUP_CC])
    AC_REQUIRE([_CCS_PROG_CC])
    AC_REQUIRE([AM_PROG_CC_C_O])

    CCS_C_COMPILER_VENDOR([ccs_c_vendor])
    ...
    AS_IF([test "$ccs_cv_c_compiler_vendor" != "ibm"],
...
# undef _GNU_SOURCE
    ...
    if test "$WANT_COVERAGE" = "1"; then
        if test "$ccs_c_vendor" = "gnu" ; then
            ...
            AC_CACHE_CHECK([if $CC supports --coverage],
                      [ccs_cv_cc_coverage],
                      [AC_TRY_COMPILE([], [],
                                      [ccs_cv_cc_coverage="yes"],
                                      [ccs_cv_cc_coverage="no"])])

            if test "$ccs_cv_cc_coverage" = "yes" ; then
                CCS_COVERAGE_FLAGS="--coverage"
                CLEANFILES="*.gcno ${CLEANFILES}"
                CONFIG_CLEAN_FILES="*.gcda *.gcov ${CONFIG_CLEAN_FILES}"
            else
                CCS_COVERAGE_FLAGS="-ftest-coverage -fprofile-arcs"
                CLEANFILES="*.bb *.bbg ${CLEANFILES}"
                CONFIG_CLEAN_FILES="*.da *.*.gcov ${CONFIG_CLEAN_FILES}"
            fi
            CFLAGS="$CFLAGS_orig $CCS_COVERAGE_FLAGS"
            LDFLAGS="$LDFLAGS_orig $CCS_COVERAGE_FLAGS"
            WRAPPER_EXTRA_CFLAGS="${WRAPPER_EXTRA_CFLAGS} $CCS_COVERAGE_FLAGS"
            WRAPPER_EXTRA_LDFLAGS="${WRAPPER_EXTRA_LDFLAGS} $CCS_COVERAGE_FLAGS"

            CCS_UNIQ(CFLAGS)
            CCS_UNIQ(LDFLAGS)
            CCS_UNIQ(WRAPPER_EXTRA_CFLAGS)
            CCS_UNIQ(WRAPPER_EXTRA_LDFLAGS)
            AC_MSG_WARN([$CCS_COVERAGE_FLAGS has been added to CFLAGS (--enable-coverage)])
    ...
    CCS_CFLAGS_BEFORE_PICKY="$CFLAGS"
    if test "$WANT_PICKY_COMPILER" = 1 -a "$ccs_c_vendor" = "gnu" ; then
        ...
        CFLAGS="$CFLAGS_orig"
        if test "$ccs_cv_cc_wno_long_double" = "yes" ; then
    ...
    if test "$GCC" = "yes"; then
        ...
        AC_CACHE_CHECK([if $CC supports -finline-functions],
                   [ccs_cv_cc_finline_functions],
                   [AC_TRY_COMPILE([], [],
                                   [ccs_cv_cc_finline_functions="yes"],
                                   [ccs_cv_cc_finline_functions="no"])])
        if test "$ccs_cv_cc_finline_functions" = "yes" ; then
        ...
        AC_CACHE_CHECK([if $CC supports -fno-strict-aliasing],
                   [ccs_cv_cc_fno_strict_aliasing],
                   [AC_TRY_COMPILE([], [],
                                   [ccs_cv_cc_fno_strict_aliasing="yes"],
                                   [ccs_cv_cc_fno_strict_aliasing="no"])])
        if test "$ccs_cv_cc_fno_strict_aliasing" = "yes" ; then
    ...
    if test ! -z "$RESTRICT_CFLAGS" ; then
        ...
        AC_CACHE_CHECK([if $CC supports $RESTRICT_CFLAGS],
                   [ccs_cv_cc_restrict_cflags],
                   [AC_TRY_COMPILE([], [], 
                                   [ccs_cv_cc_restrict_cflags="yes"],
                                   [ccs_cv_cc_restrict_cflags="no"])])
        if test "$ccs_cv_cc_restrict_cflags" = "yes" ; then
            add=" $RESTRICT_CFLAGS"
        fi
    ...
    AC_CACHE_CHECK([if $CC supports __builtin_expect],
        [ccs_cv_cc_supports___builtin_expect],
        [AC_TRY_LINK([],
          [void *ptr = (void*) 0;
           if (__builtin_expect (ptr != (void*) 0, 1)) return 0;],
          [ccs_cv_cc_supports___builtin_expect="yes"],
          [ccs_cv_cc_supports___builtin_expect="no"])])
    if test "$ccs_cv_cc_supports___builtin_expect" = "yes" ; then
        have_cc_builtin_expect=1
    else
        have_cc_builtin_expect=0
    fi
    AC_DEFINE_UNQUOTED([CCS_C_HAVE_BUILTIN_EXPECT], [$have_cc_builtin_expect],
          [Whether C compiler supports __builtin_expect])

    # see if the C compiler supports __builtin_prefetch
    AC_CACHE_CHECK([if $CC supports __builtin_prefetch],
        [ccs_cv_cc_supports___builtin_prefetch],
        [AC_TRY_LINK([],
          [int ptr;
           __builtin_prefetch(&ptr,0,0);],
          [ccs_cv_cc_supports___builtin_prefetch="yes"],
          [ccs_cv_cc_supports___builtin_prefetch="no"])])
    if test "$ccs_cv_cc_supports___builtin_prefetch" = "yes" ; then
        have_cc_builtin_prefetch=1
    else
        have_cc_builtin_prefetch=0
    fi
    AC_DEFINE_UNQUOTED([CCS_C_HAVE_BUILTIN_PREFETCH], [$have_cc_builtin_prefetch],
    ...
    CCS_ENSURE_CONTAINS_OPTFLAGS("$CCS_CFLAGS_BEFORE_PICKY")
    CCS_CFLAGS_BEFORE_PICKY="$co_result"

    AC_MSG_CHECKING([for C optimization flags])
    CCS_ENSURE_CONTAINS_OPTFLAGS(["$CFLAGS"])
    AC_MSG_RESULT([$co_result])
    CFLAGS="$co_result"
])


AC_DEFUN([_CCS_START_SETUP_CC],[
    ccs_show_subtitle "C compiler and preprocessor" 
...
AC_DEFUN([_CCS_PROG_CC],[
    #
    # Check for the compiler
    #
    ccs_cflags_save="$CFLAGS"
    AC_PROG_CC
    BASECC="`basename $CC`"
    CFLAGS="$ccs_cflags_save"
    AC_DEFINE_UNQUOTED(CCS_CC, "$CC", [OMPI underlying C compiler])
    set dummy $CC
    ccs_cc_argv0=[$]2
    CCS_WHICH([$ccs_cc_argv0], [CCS_CC_ABSOLUTE])
    AC_SUBST(CCS_CC_ABSOLUTE)
])
