...
dnl OCOMS_TRY_ASSEMBLE(asm-code, [action-if-success], [action-if-fail])
dnl
...
AC_DEFUN([OCOMS_TRY_ASSEMBLE],
[cat >conftest.s <<EOF
[$1]
EOF
if test "$CC" = "$CCAS" ; then
    ocoms_assemble="$CCAS $CCASFLAGS -c conftest.s >conftest.out 2>&1"
else
    ocoms_assemble="$CCAS $CCASFLAGS -o conftest.o conftest.s >conftest.out 2>&1"
fi
if AC_TRY_EVAL(ocoms_assemble); then
  # save the warnings
