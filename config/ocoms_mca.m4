...
*** list of type-component pairs.  Invalid input detected.])
    ...
    MCA_SETUP_DIRECT_CALL(ocoms, pml)
    MCA_SETUP_DIRECT_CALL(ocoms, mtl)
...
AC_DEFUN([MCA_CONFIGURE_PROJECT],[
    ...
    ocoms_show_subtitle "Configuring MCA for $1"
...
AC_DEFUN([MCA_ORDER_COMPONENT_LIST], [
    ...
    m4_define([component_list], 
              [esyscmd([config/ocoms_mca_priority_sort.pl] m4_foreach([mca_component], [mca_$1_$2_m4_config_component_list],
                        [m4_ifval(mca_component, [mca_component ]OCOMS_EVAL_ARG([MCA_]$1[_]$2[_]mca_component[_PRIORITY ]))]))])
])
...
AC_DEFUN([MCA_CONFIGURE_FRAMEWORK],[
    ocoms_show_subsubtitle "Configuring MCA framework $2"
    ...
    # reasonable message.
    m4_if(OCOMS_EVAL_ARG([MCA_$1_$2_CONFIGURE_MODE]), [STOP_AT_FIRST],
    ...
    m4_if(OCOMS_EVAL_ARG([MCA_$1_$2_CONFIGURE_MODE]), [STOP_AT_FIRST_PRIORITY],
          [m4_ifval(mca_$1_$2_no_config_component_list,
                   [m4_fatal([Framework $2 using STOP_AT_FIRST_PRIORITY but at least one component has no configure.m4])])])
    m4_if(OCOMS_EVAL_ARG([MCA_$1_$2_CONFIGURE_MODE]), [PRIORITY],
          [m4_ifval(mca_$1_$2_no_config_component_list,
    ...
    m4_if(OCOMS_EVAL_ARG([MCA_$1_$2_CONFIGURE_MODE]), [STOP_AT_FIRST], [MCA_ORDER_COMPONENT_LIST($1, $2)],
          [m4_if(OCOMS_EVAL_ARG([MCA_$1_$2_CONFIGURE_MODE]), [STOP_AT_FIRST_PRIORITY], [MCA_ORDER_COMPONENT_LIST($1, $2)],
                [m4_if(OCOMS_EVAL_ARG([MCA_$1_$2_CONFIGURE_MODE]), [PRIORITY], [MCA_ORDER_COMPONENT_LIST($1, $2)],
    ...
    m4_foreach(mca_component, [component_list],
               [m4_ifval(mca_component,
                  [m4_if(OCOMS_EVAL_ARG([MCA_$1_$2_CONFIGURE_MODE]), [STOP_AT_FIRST_PRIORITY],
                   ...
                   m4_if(OCOMS_EVAL_ARG([MCA_$1_$2_CONFIGURE_MODE]), [STOP_AT_FIRST_PRIORITY],
                         [AS_IF([test $components_last_result -eq 1],
                             [best_mca_component_priority=]OCOMS_EVAL_ARG([MCA_$1_$2_]mca_component[_PRIORITY]))])
                   ])])

    # configure components that provide their own configure script.
    # It would be really hard to run these for "find first that
    # works", so we don't :)
    m4_if(OCOMS_EVAL_ARG([MCA_$1_]$2[_CONFIGURE_MODE]), [STOP_AT_FIRST], [],
        [m4_if(OCOMS_EVAL_ARG([MCA_$1_]$2[_CONFIGURE_MODE]), [STOP_AT_FIRST_PRIORITY], [],
             [m4_if(OCOMS_EVAL_ARG([MCA_$1_]$2[_CONFIGURE_MODE]), [PRIORITY], [],
    ...
    OCOMS_MCA_MAKE_DIR_LIST(MCA_$1_$2_ALL_SUBDIRS, $2, [$all_components])
    OCOMS_MCA_MAKE_DIR_LIST(MCA_$1_$2_STATIC_SUBDIRS, $2, [$static_components])
    OCOMS_MCA_MAKE_DIR_LIST(MCA_$1_$2_DSO_SUBDIRS, $2, [$dso_components])
...
const ocoms_mca_base_component_t *mca_$2_base_static_components[[]] = {
`cat $outfile.struct`
  NULL
};
...
AC_DEFUN([MCA_CONFIGURE_NO_CONFIG_COMPONENT],[
    ocoms_show_subsubsubtitle "MCA component $2:$3 (no configuration)"
...
AC_DEFUN([MCA_CONFIGURE_M4_CONFIG_COMPONENT],[
    m4_ifdef([MCA_$1_$2_$3_PRIORITY],
        [ocoms_show_subsubsubtitle "MCA component $2:$3 (m4 configuration macro, priority MCA_$1_$2_$3_PRIORITY)"],
        [ocoms_show_subsubsubtitle "MCA component $2:$3 (m4 configuration macro)"])
...
AC_DEFUN([MCA_CONFIGURE_ALL_CONFIG_COMPONENTS],[
    for component_path in $srcdir/$1/mca/$2/* ; do
        ...
        if test -d $component_path -a -x $component_path/configure ; then
            ocoms_show_subsubsubtitle "MCA component $2:$component (need to configure)"
            ...
            if test "$should_build" = "1" ; then
                OCOMS_CONFIG_SUBDIR([$1/mca/$2/$component],
                                   [$ocoms_subdir_args],
                                   [should_build=1], [should_build=0])
            fi
...
AC_DEFUN([MCA_PROCESS_COMPONENT],[
    ...
    if test "$8" = "dso" ; then
        ...
        echo "extern const ocoms_mca_base_component_t mca_${framework}_${component}_component;" >> $outfile.extern
...
AC_DEFUN([MCA_COMPONENT_BUILD_CHECK],[
    ...
    # - there is no ocoms_ignore file
    # - there is an ocoms_ignore, but there is an empty ocoms_unignore
    # - there is an ocoms_ignore, but username is in ocoms_unignore
    if test -d $component_path ; then
        ...
        want_component=1
        if test -f $component_path/.ocoms_ignore ; then
            # If there is an ocoms_ignore file, don't build
            # the component.  Note that this decision can be
            # overridden by the unignore logic below.
            want_component=0
        fi
        if test -f $component_path/.ocoms_unignore ; then
            # if there is an empty ocoms_unignore, that is
            # equivalent to having your userid in the unignore file.
            # If userid is in the file, unignore the ignore file.
            if test ! -s $component_path/.ocoms_unignore ; then
                want_component=1
            elif test ! -z "`$GREP $OPAL_CONFIGURE_USER $component_path/.ocoms_unignore`" ; then
                want_component=1
            fi
        fi
...
# OCOMS_MCA_MAKE_DIR_LIST(subst'ed variable, framework, shell list)
# -------------------------------------------------------------------------
AC_DEFUN([OCOMS_MCA_MAKE_DIR_LIST],[
    $1=
    for item in $3 ; do
       $1="$$1 mca/$2/$item"
    done
