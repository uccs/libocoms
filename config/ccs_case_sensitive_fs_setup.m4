...
AC_DEFUN([CCS_CASE_SENSITIVE_FS_SETUP],[
#
# Arguments: none
#
# Dependencies: None
...
if test "$with_cs_fs" = "yes"; then
    ...
    CCS_WANT_CS_FS=$have_cs_fs
else
    CCS_WANT_CS_FS=0
fi

if test "$CCS_WANT_CS_FS" = "1"; then
    AC_MSG_RESULT([yes])
else
    AC_MSG_RESULT([no])
fi

AM_CONDITIONAL(CASE_SENSITIVE_FS, test "$CCS_WANT_CS_FS" = "1")

if test "$CCS_WANT_CS_FS" = "0"; then
	cat <<EOF
