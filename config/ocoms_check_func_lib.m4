...
# OCOMS_CHECK_FUNC_LIB(func, lib, [action-if-found], [action-if-not-found])
# ------------------------------
...
AC_DEFUN([OCOMS_CHECK_FUNC_LIB],[
    AS_VAR_PUSHDEF([ocoms_var], [ocoms_cv_func_lib_$1_$2])dnl
    AC_CACHE_CHECK([if we need -l$2 for $1],
        ocoms_var,
        [AC_LINK_IFELSE([AC_LANG_FUNC_LINK_TRY([$1])],
            [AS_VAR_SET(ocoms_var, "no")],
            [LIBS_save="$LIBS"
             LIBS="$LIBS -l$2"
             AC_LINK_IFELSE([AC_LANG_FUNC_LINK_TRY([$1])],
                 [AS_VAR_SET(ocoms_var, "yes")],
                 [AS_VAR_SET(ocoms_var, "not found")])
             LIBS="$LIBS_save"])])
    AS_VAR_IF(ocoms_var, [yes],
              [LIBS="$LIBS -l$2"])
