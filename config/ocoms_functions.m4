...
AC_DEFUN([OCOMS_CONFIGURE_SETUP],[
...
OCOMS_CONFIGURE_USER="`whoami`"
OCOMS_CONFIGURE_HOST="`hostname | head -n 1`"
OCOMS_CONFIGURE_DATE="`date`"

#
# Save these details so that they can be used in ocoms_info later
#
AC_SUBST(OCOMS_CONFIGURE_USER)
AC_SUBST(OCOMS_CONFIGURE_HOST)
AC_SUBST(OCOMS_CONFIGURE_DATE)])dnl

dnl #######################################################################
dnl #######################################################################
dnl #######################################################################

AC_DEFUN([OCOMS_BASIC_SETUP],[
#
# Save some stats about this build
#
...
# Stupid autoconf 2.54 has a bug in AC_PREFIX_PROGRAM -- if ocoms_clean
# is not found in the path and the user did not specify --prefix,
# we'll get a $prefix of "."

ocoms_prefix_save="$prefix"
AC_PREFIX_PROGRAM(ocoms_clean)
if test "$prefix" = "."; then
    prefix="$ocoms_prefix_save"
fi
unset ocoms_prefix_save
...
AC_ARG_ENABLE(dist, 
    ...
    OCOMS_WANT_DIST=yes, OCOMS_WANT_DIST=no)

if test "$OCOMS_WANT_DIST" = "yes"; then
...
$1 1>&5 2>&1
ocoms_status=$?
OCOMS_LOG_MSG([\$? = $ocoms_status], 1)
if test "$ocoms_status" = "0"; then
    unset ocoms_status
    $2
else
    unset ocoms_status
...
for val in ${$1}; do
    ...
    ocoms_done="`expr $ocoms_i \> $ocoms_count`"
    while test "$ocoms_found" = "0" -a "$ocoms_done" = "0"; do

	# Have we seen this token already?  Prefix the comparison with
	# "x" so that "-Lfoo" values won't be cause an error.

	ocoms_eval="expr x$val = x\$ocoms_array_$ocoms_i"
	ocoms_found=`eval $ocoms_eval`
	...
	ocoms_i="`expr $ocoms_i + 1`"
    done

    # If we didn't find the token, add it to the "array"

    if test "$ocoms_found" = "0"; then
	ocoms_eval="ocoms_array_$ocoms_i=$val"
	eval $ocoms_eval
	ocoms_count="`expr $ocoms_count + 1`"
    else
	ocoms_i="`expr $ocoms_i - 1`"
    fi
done
...
ocoms_done="`expr $ocoms_i \> $ocoms_count`"
ocoms_newval=
while test "$ocoms_done" = "0"; do
    ocoms_eval="ocoms_newval=\"$ocoms_newval \$ocoms_array_$ocoms_i\""
    eval $ocoms_eval

    ocoms_eval="unset ocoms_array_$ocoms_i"
    eval $ocoms_eval
...
ocoms_newval="`echo $ocoms_newval`"
ocoms_eval="$ocoms_name=\"$ocoms_newval\""
eval $ocoms_eval

# Clean up

unset ocoms_name ocoms_i ocoms_done ocoms_newval ocoms_eval ocoms_count])dnl
...
AC_DEFUN([OCOMS_WHICH],[
# 1 is the variable name to do "which" on
# 2 is the variable name to assign the return value to

OCOMS_VAR_SCOPE_PUSH([ocoms_prog ocoms_file ocoms_dir ocoms_sentinel])

ocoms_prog=$1

IFS_SAVE=$IFS
IFS="$PATH_SEPARATOR"
for ocoms_dir in $PATH; do
    if test -x "$ocoms_dir/$ocoms_prog"; then
        $2="$ocoms_dir/$ocoms_prog"
        break
    fi
done
IFS=$IFS_SAVE
...
# Declare some variables; use OCOMS_VAR_SCOPE_END to ensure that they
# are cleaned up / undefined.
AC_DEFUN([OCOMS_VAR_SCOPE_PUSH],[

    # Is the private index set?  If not, set it.
    if test "x$ocoms_scope_index" = "x"; then
        ocoms_scope_index=1
    fi
    ...
    for ocoms_var in $1; do
        ocoms_str="ocoms_str=\"\$$ocoms_var\""
        eval $ocoms_str

        if test "x$ocoms_str" != "x"; then
            AC_MSG_WARN([Found configure shell variable clash!])
            AC_MSG_WARN([[OCOMS_VAR_SCOPE_PUSH] called on "$ocoms_var",])
            AC_MSG_WARN([but it is already defined with value "$ocoms_str"])
    ...
    ocoms_str="ocoms_scope_$ocoms_scope_index=\"$1\""
    eval $ocoms_str
    unset ocoms_str

    env | grep ocoms_scope
    ocoms_scope_index=`expr $ocoms_scope_index + 1`
])dnl

# Unset a bunch of variables that were previously set
AC_DEFUN([OCOMS_VAR_SCOPE_POP],[
    # Unwind the index
    ocoms_scope_index=`expr $ocoms_scope_index - 1`
    ocoms_scope_test=`expr $ocoms_scope_index \> 0`
    if test "$ocoms_scope_test" = "0"; then
        AC_MSG_WARN([[OCOMS_VAR_SCOPE_POP] popped too many OMPI configure scopes.])
        AC_MSG_WARN([This usually indicates an error in configure.])
        AC_MSG_ERROR([Cannot continue])
    fi

    # Get the variable names from that index
    ocoms_str="ocoms_str=\"\$ocoms_scope_$ocoms_scope_index\""
    eval $ocoms_str

    # Iterate over all the variables and unset them all
    for ocoms_var in $ocoms_str; do
        unset $ocoms_var
    done
])dnl
...
#
# OCOMS_WITH_OPTION_MIN_MAX_VALUE(NAME,DEFAULT_VALUE,LOWER_BOUND,UPPER_BOUND)
# Defines a variable OCOMS_MAX_xxx, with "xxx" being specified as parameter $1 as "variable_name".
...
#
AC_DEFUN([OCOMS_WITH_OPTION_MIN_MAX_VALUE], [
    max_value=[$2]
    ...
    AC_DEFINE_UNQUOTED([OCOMS_MAX_]m4_toupper($1), $max_value,
                       [Maximum length of ]m4_translit($1, [_], [ ])[s (default is $2)])
    [OCOMS_MAX_]m4_toupper($1)=$max_value
    AC_SUBST([OCOMS_MAX_]m4_toupper($1))
])dnl
