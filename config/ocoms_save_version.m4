...
dnl
dnl This file is also used as input to ocoms_get_version.sh.
dnl
...
#  prefix_REPO_REV
#  prefix_RELEASE_DATE
m4_define([OCOMS_GET_VERSION],[
    : ${ocoms_ver_need_repo_rev=1}
    : ${svnversion_result=-1}
	...
	[eval] "$ocoms_vers"
        ...
        if test $$2_WANT_REPO_REV -eq 1 && test $ocoms_ver_need_repo_rev -eq 1 ; then
...
# OCOMS_SAVE_VERSION(project_short, project-long, 
#                  version_file, header_file])
# ----------------------------------------------
# creates version information for project from version_file, using
# OCOMS_GET_VERSION().  Information is AC_SUBSTed and put in
# header_file.
AC_DEFUN([OCOMS_SAVE_VERSION], [
    OCOMS_GET_VERSION([$3], [$1])
