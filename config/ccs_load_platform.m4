...
AC_DEFUN([CCS_LOAD_PLATFORM], [
    ...
    if test "$with_platform" = "yes" ; then
        ...
        CCS_LOG_MSG([Loading environment file $with_platform, with contents below])
        CCS_LOG_FILE([$with_platform])
        . "$with_platform"

        # see if they left us a name
        if test "$CCS_PLATFORM_LOADED" != "" ; then
           platform_loaded="$CCS_PLATFORM_LOADED"
        else
           platform_loaded="$with_platform"
        fi
        echo "Loaded platform arguments for $platform_loaded"
        CCS_LOG_MSG([Loaded platform arguments for $platform_loaded])
        ...
        if test -r "${platform_file_dir}/${platform_alt_mca_file}" ; then
            AC_SUBST(CCS_DEFAULT_MCA_PARAM_CONF, [$platform_file_dir/$platform_alt_mca_file])
            AC_SUBST(CCS_PARAM_FROM_PLATFORM, "yes")
        # if not, see if a file is there with the default name
        elif test -r "${platform_file_dir}/openmpi-mca-params.conf" ; then
            AC_SUBST(CCS_DEFAULT_MCA_PARAM_CONF, [$platform_file_dir/openmpi-mca-params.conf])
            AC_SUBST(CCS_PARAM_FROM_PLATFORM, "yes")
        # if not, then just use the default
        else
            AC_SUBST(CCS_DEFAULT_MCA_PARAM_CONF, [openmpi-mca-params.conf])
            AC_SUBST(CCS_PARAM_FROM_PLATFORM, "no")
        fi
