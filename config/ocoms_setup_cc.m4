...
AC_DEFUN([OCOMS_SETUP_CC],[
    ...
    AC_REQUIRE([_OCOMS_START_SETUP_CC])
    AC_REQUIRE([_OCOMS_PROG_CC])
    AC_REQUIRE([AM_PROG_CC_C_O])

    OCOMS_C_COMPILER_VENDOR([ocoms_c_vendor])
    ...
    AS_IF([test "$ocoms_cv_c_compiler_vendor" != "ibm"],
...
# undef _GNU_SOURCE
    ...
    if test "$WANT_COVERAGE" = "1"; then
        if test "$ocoms_c_vendor" = "gnu" ; then
            ...
            OCOMS_COVERAGE_FLAGS=

            AC_CACHE_CHECK([if $CC supports --coverage],
                      [ocoms_cv_cc_coverage],
                      [AC_TRY_COMPILE([], [],
                                      [ocoms_cv_cc_coverage="yes"],
                                      [ocoms_cv_cc_coverage="no"])])

            if test "$ocoms_cv_cc_coverage" = "yes" ; then
                OCOMS_COVERAGE_FLAGS="--coverage"
                CLEANFILES="*.gcno ${CLEANFILES}"
                CONFIG_CLEAN_FILES="*.gcda *.gcov ${CONFIG_CLEAN_FILES}"
            else
                OCOMS_COVERAGE_FLAGS="-ftest-coverage -fprofile-arcs"
                CLEANFILES="*.bb *.bbg ${CLEANFILES}"
                CONFIG_CLEAN_FILES="*.da *.*.gcov ${CONFIG_CLEAN_FILES}"
            fi
            CFLAGS="$CFLAGS_orig $OCOMS_COVERAGE_FLAGS"
            LDFLAGS="$LDFLAGS_orig $OCOMS_COVERAGE_FLAGS"
            WRAPPER_EXTRA_CFLAGS="${WRAPPER_EXTRA_CFLAGS} $OCOMS_COVERAGE_FLAGS"
            WRAPPER_EXTRA_LDFLAGS="${WRAPPER_EXTRA_LDFLAGS} $OCOMS_COVERAGE_FLAGS"
    ...
    OCOMS_CFLAGS_BEFORE_PICKY="$CFLAGS"
    if test "$WANT_PICKY_COMPILER" = 1 -a "$ocoms_c_vendor" = "gnu" ; then
        ...
        CFLAGS="$CFLAGS_orig"
        if test "$ocoms_cv_cc_wno_long_double" = "yes" ; then
    ...
    if test "$GCC" = "yes"; then
        ...
        AC_CACHE_CHECK([if $CC supports -finline-functions],
                   [ocoms_cv_cc_finline_functions],
                   [AC_TRY_COMPILE([], [],
                                   [ocoms_cv_cc_finline_functions="yes"],
                                   [ocoms_cv_cc_finline_functions="no"])])
        if test "$ocoms_cv_cc_finline_functions" = "yes" ; then
        ...
        AC_CACHE_CHECK([if $CC supports -fno-strict-aliasing],
                   [ocoms_cv_cc_fno_strict_aliasing],
                   [AC_TRY_COMPILE([], [],
                                   [ocoms_cv_cc_fno_strict_aliasing="yes"],
                                   [ocoms_cv_cc_fno_strict_aliasing="no"])])
        if test "$ocoms_cv_cc_fno_strict_aliasing" = "yes" ; then
    ...
    if test ! -z "$RESTRICT_CFLAGS" ; then
        ...
        AC_CACHE_CHECK([if $CC supports $RESTRICT_CFLAGS],
                   [ocoms_cv_cc_restrict_cflags],
                   [AC_TRY_COMPILE([], [], 
                                   [ocoms_cv_cc_restrict_cflags="yes"],
                                   [ocoms_cv_cc_restrict_cflags="no"])])
        if test "$ocoms_cv_cc_restrict_cflags" = "yes" ; then
            add=" $RESTRICT_CFLAGS"
        fi
    ...
    AC_CACHE_CHECK([if $CC supports __builtin_expect],
        [ocoms_cv_cc_supports___builtin_expect],
        [AC_TRY_LINK([],
          [void *ptr = (void*) 0;
           if (__builtin_expect (ptr != (void*) 0, 1)) return 0;],
          [ocoms_cv_cc_supports___builtin_expect="yes"],
          [ocoms_cv_cc_supports___builtin_expect="no"])])
    if test "$ocoms_cv_cc_supports___builtin_expect" = "yes" ; then
        have_cc_builtin_expect=1
    else
        have_cc_builtin_expect=0
    fi
    AC_DEFINE_UNQUOTED([OCOMS_C_HAVE_BUILTIN_EXPECT], [$have_cc_builtin_expect],
          [Whether C compiler supports __builtin_expect])

    # see if the C compiler supports __builtin_prefetch
    AC_CACHE_CHECK([if $CC supports __builtin_prefetch],
        [ocoms_cv_cc_supports___builtin_prefetch],
        [AC_TRY_LINK([],
          [int ptr;
           __builtin_prefetch(&ptr,0,0);],
          [ocoms_cv_cc_supports___builtin_prefetch="yes"],
          [ocoms_cv_cc_supports___builtin_prefetch="no"])])
    if test "$ocoms_cv_cc_supports___builtin_prefetch" = "yes" ; then
        have_cc_builtin_prefetch=1
    else
        have_cc_builtin_prefetch=0
    fi
    AC_DEFINE_UNQUOTED([OCOMS_C_HAVE_BUILTIN_PREFETCH], [$have_cc_builtin_prefetch],
    ...
    OCOMS_ENSURE_CONTAINS_OPTFLAGS("$OCOMS_CFLAGS_BEFORE_PICKY")
    OCOMS_CFLAGS_BEFORE_PICKY="$co_result"

    AC_MSG_CHECKING([for C optimization flags])
    OCOMS_ENSURE_CONTAINS_OPTFLAGS(["$CFLAGS"])
    AC_MSG_RESULT([$co_result])
    CFLAGS="$co_result"
])


AC_DEFUN([_OCOMS_START_SETUP_CC],[
...
AC_DEFUN([_OCOMS_PROG_CC],[
    #
    # Check for the compiler
    #
    ocoms_cflags_save="$CFLAGS"
    AC_PROG_CC
    BASECC="`basename $CC`"
    CFLAGS="$ocoms_cflags_save"
    AC_DEFINE_UNQUOTED(OCOMS_CC, "$CC", [OMPI underlying C compiler])
    set dummy $CC
    ocoms_cc_argv0=[$]2
    OCOMS_WHICH([$ocoms_cc_argv0], [OCOMS_CC_ABSOLUTE])
    AC_SUBST(OCOMS_CC_ABSOLUTE)
])
