...
# OCOMS_CHECK_OPENIB(prefix, [action-if-found], [action-if-not-found])
# --------------------------------------------------------
# check if OPENIB support can be found.  sets prefix_{CPPFLAGS, 
# LDFLAGS, LIBS} as needed and runs action-if-found if there is
# support, otherwise executes action-if-not-found
AC_DEFUN([OCOMS_CHECK_OPENIB],[
    OCOMS_VAR_SCOPE_PUSH([$1_msg])
    ...
    OCOMS_CHECK_WITHDIR([openib], [$with_openib], [include/infiniband/verbs.h])
    AC_ARG_WITH([openib-libdir],
       [AC_HELP_STRING([--with-openib-libdir=DIR],
             [Search for OpenFabrics libraries in DIR])])
    OCOMS_CHECK_WITHDIR([openib-libdir], [$with_openib_libdir], [libibverbs.*])
    ...
    if test "$enable_openib_control_hdr_padding" = "yes"; then
        AC_MSG_RESULT([yes])
        ocoms_openib_pad_hdr=1
    else
        AC_MSG_RESULT([no])
        ocoms_openib_pad_hdr=0
    fi
    AC_DEFINE_UNQUOTED([OCOMS_OPENIB_PAD_HDR], [$ocoms_openib_pad_hdr],
...
dnl                        [enable_openib_ibcm="$enableval"], [enable_openib_ibcm="no"])
    ...
    AS_IF([test ! -z "$with_openib" -a "$with_openib" != "yes"],
          [ocoms_check_openib_dir="$with_openib"])
    AS_IF([test ! -z "$with_openib_libdir" -a "$with_openib_libdir" != "yes"],
          [ocoms_check_openib_libdir="$with_openib_libdir"])
    AS_IF([test "$with_openib" = "no"],
          [ocoms_check_openib_happy="no"],
          [ocoms_check_openib_happy="yes"])

    ocoms_check_openib_$1_save_CPPFLAGS="$CPPFLAGS"
    ocoms_check_openib_$1_save_LDFLAGS="$LDFLAGS"
    ocoms_check_openib_$1_save_LIBS="$LIBS"

    AS_IF([test "$ocoms_check_openib_happy" = "yes"],
    ...
    AS_IF([test "$ocoms_check_openib_happy" = "yes"], 
          [OCOMS_CHECK_PACKAGE([$1],
                              ...
                              [ibv_open_device],
                              [],
                              [$ocoms_check_openib_dir],
                              [$ocoms_check_openib_libdir],
                              [ocoms_check_openib_happy="yes"],
                              [ocoms_check_openib_happy="no"])])
    ...
    AS_IF([test "$ocoms_check_openib_happy" = "yes"],
          [AC_CACHE_CHECK(
              ...
              [ocoms_cv_func_ibv_create_cq_args],
              [AC_LINK_IFELSE(
                 [AC_LANG_PROGRAM(
                    [[#include <infiniband/verbs.h> ]],
                    [[ibv_create_cq(NULL, 0, NULL, NULL, 0);]])],
                 [ocoms_cv_func_ibv_create_cq_args=5],
                 [AC_LINK_IFELSE(
                    [AC_LANG_PROGRAM(
                       [[#include <infiniband/verbs.h> ]],
                       [[ibv_create_cq(NULL, 0, NULL);]])],
                    [ocoms_cv_func_ibv_create_cq_args=3],
                    [ocoms_cv_func_ibv_create_cq_args="unknown"])])])
           AS_IF([test "$ocoms_cv_func_ibv_create_cq_args" = "unknown"],
                 [AC_MSG_WARN([Can not determine number of args to ibv_create_cq.])
                  AC_MSG_WARN([Not building component.])
                  ocoms_check_openib_happy="no"],
                 [AC_DEFINE_UNQUOTED([OCOMS_IBV_CREATE_CQ_ARGS],
                                     [$ocoms_cv_func_ibv_create_cq_args],
                                     [Number of arguments to ibv_create_cq])])])
...
dnl           fi
           ...
           if test "1" = "$$1_have_rdmaoe"; then
                AC_DEFINE_UNQUOTED([OCOMS_HAVE_RDMAOE], [$$1_have_rdmaoe], [Enable RDMAoE support])
                AC_MSG_RESULT([yes])
           else
                AC_MSG_RESULT([no])
           fi
    ...
    AC_DEFINE_UNQUOTED([OCOMS_HAVE_CONNECTX_XRC], [$$1_have_xrc],
    ...
    AC_DEFINE_UNQUOTED([OCOMS_HAVE_RDMACM], [$$1_have_rdmacm],
    ...
    AC_DEFINE_UNQUOTED([OCOMS_HAVE_IBCM], [$$1_have_ibcm],
    ...
    CPPFLAGS="$ocoms_check_openib_$1_save_CPPFLAGS"
    LDFLAGS="$ocoms_check_openib_$1_save_LDFLAGS"
    LIBS="$ocoms_check_openib_$1_save_LIBS"
