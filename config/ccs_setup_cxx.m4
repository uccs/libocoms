...
AC_DEFUN([CCS_SETUP_CXX_BANNER],[
    ccs_show_subtitle "C++ compiler and preprocessor" 
])

# This macro is necessary because PROG_CXX* is REQUIREd by multiple
# places in SETUP_CXX.
AC_DEFUN([CCS_PROG_CXX],[
    CCS_VAR_SCOPE_PUSH([ccs_cxxflags_save])
    ccs_cxxflags_save="$CXXFLAGS"
    AC_PROG_CXX
    AC_PROG_CXXCPP
    CXXFLAGS="$ccs_cxxflags_save"
    CCS_VAR_SCOPE_POP
])
...
# this macro.
AC_DEFUN([CCS_SETUP_CXX],[
    AC_REQUIRE([CCS_SETUP_CXX_BANNER])

    _CCS_SETUP_CXX_COMPILER

    CCS_CXX_COMPILER_VENDOR([ccs_cxx_vendor])

    _CCS_SETUP_CXX_COMPILER_BACKEND
])

# _CCS_SETUP_CXX_COMPILER()
# --------------------------
# Setup the CXX compiler
AC_DEFUN([_CCS_SETUP_CXX_COMPILER],[
    CCS_VAR_SCOPE_PUSH(ccs_cxx_compiler_works)

    # Must REQUIRE the PROG_CXX macro and not call it directly here for
    # reasons well-described in the AC2.64 (and beyond) docs.
    AC_REQUIRE([CCS_PROG_CXX])
    BASECXX="`basename $CXX`"

    AS_IF([test "x$CXX" = "x"], [CXX=none])
    set dummy $CXX
    ccs_cxx_argv0=[$]2
    CCS_WHICH([$ccs_cxx_argv0], [CCS_CXX_ABSOLUTE])
    AS_IF([test "x$CCS_CXX_ABSOLUTE" = "x"], [CCS_CXX_ABSOLUTE=none])

    AC_DEFINE_UNQUOTED(CCS_CXX, "$CXX", [CCS underlying C++ compiler])
    AC_SUBST(CCS_CXX_ABSOLUTE)
...
# _CCS_SETUP_CXX_COMPILER_BACKEND()
# ----------------------------------
# Back end of _CCS_SETUP_CXX_COMPILER_BACKEND()
AC_DEFUN([_CCS_SETUP_CXX_COMPILER_BACKEND],[
    # Do we want code coverage
    if test "$WANT_COVERAGE" = "1"; then 
        if test "$ccs_cxx_vendor" = "gnu" ; then
            AC_MSG_WARN([$CCS_COVERAGE_FLAGS has been added to CFLAGS (--enable-coverage)])
            WANT_DEBUG=1
            CXXFLAGS="${CXXFLAGS} $CCS_COVERAGE_FLAGS"
            WRAPPER_EXTRA_CXXFLAGS="${WRAPPER_EXTRA_CXXFLAGS} $CCS_COVERAGE_FLAGS"
        else
    ...
    CCS_CXXFLAGS_BEFORE_PICKY="$CXXFLAGS"
    if test "$WANT_PICKY_COMPILER" = 1 -a "$ccs_cxx_vendor" = "gnu"; then
        ...
        AC_LANG_POP(C++)
        if test "$ccs_cv_cxx_wno_long_double" = "yes" ; then
    ...
    if test "$GXX" = "yes"; then
        ...
        AC_CACHE_CHECK([if $CXX supports -finline-functions],
                   [ccs_cv_cxx_finline_functions],
                   [AC_TRY_COMPILE([], [],
                                   [ccs_cv_cxx_finline_functions="yes"],
                                   [ccs_cv_cxx_finline_functions="no"])])
        if test "$ccs_cv_cxx_finline_functions" = "yes" ; then
    ...
    if[ test "$ccs_cv_cxx_compiler_vendor" != "microsoft" ]; then
      CCS_LANG_LINK_WITH_C([C++], [],
        [cat <<EOF >&2
**********************************************************************
...
EOF
    ...
    *hpux*)
    ...
    # config/ccs_ensure_contains_optflags.m4
    CCS_ENSURE_CONTAINS_OPTFLAGS(["$CXXFLAGS"])
    ...
    CCS_C_GET_ALIGNMENT(bool, CCS_ALIGNMENT_CXX_BOOL)
    AC_LANG_POP(C++)
])
