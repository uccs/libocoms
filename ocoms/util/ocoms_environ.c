...
#ifdef __WINDOWS__ 
#define OCOMS_DEFAULT_TMPDIR "C:\\TEMP" 
#else 
#define OCOMS_DEFAULT_TMPDIR "/tmp" 
#endif
 ...
 * duplicate entires
 */
char **ocoms_environ_merge(char **minor, char **major)
{
    int i;
    char **ret = NULL;
    char *name, *value;
    ...
    if (NULL == major) {
        if (NULL == minor) {
            ...
            return ocoms_argv_copy(minor);
        }
    }

    /* First, copy major */

    ret = ocoms_argv_copy(major);
    ...
    /* Now go through minor and call ocoms_setenv(), but with overwrite
       as false */

    for (i = 0; NULL != minor[i]; ++i) {
        value = strchr(minor[i], '=');
        if (NULL == value) {
            ocoms_setenv(minor[i], NULL, false, &ret);
        } else {
            ...
            *value = '\0';
            ocoms_setenv(name, value + 1, false, &ret);
            free(name);
        }
    }
 ...
 * array
 */
int ocoms_setenv(const char *name, const char *value, bool overwrite,
                char ***env)
{
    int i;
    ...
    if( NULL == env ) {
        ...
        i = 0;
        ocoms_argv_append(&i, env, newvalue);
        free(newvalue);
    ...
    if( *env == environ ) {
        /* THIS IS POTENTIALLY A MEMORY LEAK!  But I am doing it
           ...
           check the return code of ocoms_setenv() and notice that we
 ...
 * environ-like array
 */
int ocoms_unsetenv(const char *name, char ***env)
{
    int i;
    char *compare;
    size_t len;
...
const char* ocoms_tmp_directory( void )
{
    const char* str;
    ...
    if( NULL == (str = getenv("TMPDIR")) )
        if( NULL == (str = getenv("TEMP")) )
            if( NULL == (str = getenv("TMP")) )
                str = OCOMS_DEFAULT_TMPDIR;
    return str;
}

const char* ocoms_home_directory( void )
{
