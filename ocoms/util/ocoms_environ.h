...
#ifndef OCOMS_ENVIRON_H
#define OCOMS_ENVIRON_H
 ...
 * later be freed with a call to ocoms_argv_free().
 *
 ...
 */
OCOMS_DECLSPEC char **ocoms_environ_merge(char **minor, char **major) __ocoms_attribute_warn_unused_result__;
 ...
 *   ocoms_setenv("foo", "bar", true, &environ);
 * \endcode
 *
 * NOTE: If you use the real environ, ocoms_setenv() will turn
 ...
 * \code
 *   char **my_env = NULL;
 *   ocoms_setenv("foo", "bar", true, &my_env);
 * \endcode
 */
OCOMS_DECLSPEC int ocoms_setenv(const char *name, const char *value,
                              bool overwrite, char ***env) __ocoms_attribute_nonnull__(1);
 ...
 */
OCOMS_DECLSPEC int ocoms_unsetenv(const char *name, char ***env) __ocoms_attribute_nonnull__(1);

/* A consistent way to retrieve the home and tmp directory on all supported
 * platforms.
 */
OCOMS_DECLSPEC const char* ocoms_home_directory( void );
OCOMS_DECLSPEC const char* ocoms_tmp_directory( void );
