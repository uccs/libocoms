...
typedef struct ocoms_object_t ocoms_object_t;
typedef struct ocoms_class_t ocoms_class_t;
typedef void (*ocoms_construct_t) (ocoms_object_t *);
typedef void (*ocoms_destruct_t) (ocoms_object_t *);
 ...
 * definition.
 */
struct ocoms_class_t {
    const char *cls_name;           /**< symbolic name for class */
    ocoms_class_t *cls_parent;       /**< parent class descriptor */
    ocoms_construct_t cls_construct; /**< class constructor */
    ocoms_destruct_t cls_destruct;   /**< class destructor */
...
#define OBJ_CLASS_INSTANCE(NAME, PARENT, CONSTRUCTOR, DESTRUCTOR)       \
    ocoms_class_t NAME ## _class = {                                     \
        # NAME,                                                         \
        OBJ_CLASS(PARENT),                                              \
        (ocoms_construct_t) CONSTRUCTOR,                                 \
        (ocoms_destruct_t) DESTRUCTOR,                                   \
        0, 0, NULL, NULL,                                               \
        sizeof(NAME)                                                    \
    }
...
#define OBJ_CLASS_DECLARATION(NAME)             \
    extern ocoms_class_t NAME ## _class
 ...
 */
static inline ocoms_object_t *ocoms_obj_new(ocoms_class_t * cls);
#if OCOMS_ENABLE_DEBUG
static inline ocoms_object_t *ocoms_obj_new_debug(ocoms_class_t* type, const char* file, int line)
{
    ocoms_object_t* object = ocoms_obj_new(type);
...
#define OBJ_NEW(type)                                   \
    ((type *)ocoms_obj_new_debug(OBJ_CLASS(type), __FILE__, __LINE__))
#else
#define OBJ_NEW(type)                                   \
    ((type *) ocoms_obj_new(OBJ_CLASS(type)))
#endif  /* OCOMS_ENABLE_DEBUG */
...
#define OBJ_RETAIN(object)                                              \
    do {                                                                \
        assert(NULL != ((ocoms_object_t *) (object))->obj_class);        \
        assert(OCOMS_OBJ_MAGIC_ID == ((ocoms_object_t *) (object))->obj_magic_id); \
        ocoms_obj_update((ocoms_object_t *) (object), 1);                 \
        assert(((ocoms_object_t *) (object))->obj_reference_count >= 0); \
    } while (0)
#else
#define OBJ_RETAIN(object)  ocoms_obj_update((ocoms_object_t *) (object), 1);
#endif
...
#define OBJ_REMEMBER_FILE_AND_LINENO( OBJECT, FILE, LINENO )    \
    do {                                                        \
        ((ocoms_object_t*)(OBJECT))->cls_init_file_name = FILE;  \
        ((ocoms_object_t*)(OBJECT))->cls_init_lineno = LINENO;   \
    } while(0)
#define OBJ_SET_MAGIC_ID( OBJECT, VALUE )                       \
    do {                                                        \
        ((ocoms_object_t*)(OBJECT))->obj_magic_id = (VALUE);     \
    } while(0)
#else
#define OBJ_REMEMBER_FILE_AND_LINENO( OBJECT, FILE, LINENO )
#define OBJ_SET_MAGIC_ID( OBJECT, VALUE )
...
#define OBJ_RELEASE(object)                                             \
    do {                                                                \
        assert(NULL != ((ocoms_object_t *) (object))->obj_class);        \
        assert(OCOMS_OBJ_MAGIC_ID == ((ocoms_object_t *) (object))->obj_magic_id); \
        if (0 == ocoms_obj_update((ocoms_object_t *) (object), -1)) {     \
            OBJ_SET_MAGIC_ID((object), 0);                              \
            ocoms_obj_run_destructors((ocoms_object_t *) (object));       \
...
do {                                                                \
    ...
    if (0 == (type)->cls_initialized) {                             \
        ocoms_class_initialize((type));                              \
    }                                                               \
    ((ocoms_object_t *) (object))->obj_class = (type);               \
    ((ocoms_object_t *) (object))->obj_reference_count = 1;          \
    ocoms_obj_run_constructors((ocoms_object_t *) (object));          \
    OBJ_REMEMBER_FILE_AND_LINENO( object, __FILE__, __LINE__ ); \
} while (0)
...
OCOMS_DECLSPEC OBJ_CLASS_DECLARATION(ocoms_object_t);
 ...
 */
OCOMS_DECLSPEC void ocoms_class_initialize(ocoms_class_t *);
 ...
 */
OCOMS_DECLSPEC int ocoms_class_finalize(void);
 ...
 */
static inline void ocoms_obj_run_constructors(ocoms_object_t * object)
{
    ocoms_construct_t* cls_construct;
 ...
 * @param size          Pointer to the object.
 */
static inline void ocoms_obj_run_destructors(ocoms_object_t * object)
{
    ocoms_destruct_t* cls_destruct;
...
static inline ocoms_object_t *ocoms_obj_new(ocoms_class_t * cls)
{
    ...
    assert(cls->cls_sizeof >= sizeof(ocoms_object_t));

    object = (ocoms_object_t *) malloc(cls->cls_sizeof);
    if (0 == cls->cls_initialized) {
        ocoms_class_initialize(cls);
    }
    if (NULL != object) {
        object->obj_class = cls;
        object->obj_reference_count = 1;
        ocoms_obj_run_constructors(object);
    }
    return object;
}
 ...
 */
static inline int ocoms_obj_update(ocoms_object_t *object, int inc) __ocoms_attribute_always_inline__;
static inline int ocoms_obj_update(ocoms_object_t *object, int inc)
{
#if OCOMS_ENABLE_MULTI_THREADS
    return ocoms_atomic_add_32(&(object->obj_reference_count), inc );
#else
