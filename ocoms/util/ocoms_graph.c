...
#include "ocoms/util/ocoms_graph.h"
...
static void ocoms_graph_vertex_construct(ocoms_graph_vertex_t *vertex);
static void ocoms_graph_vertex_destruct(ocoms_graph_vertex_t *vertex);

OBJ_CLASS_INSTANCE(
    ocoms_graph_vertex_t,
    ocoms_list_item_t,
    ocoms_graph_vertex_construct,
    ocoms_graph_vertex_destruct
);


static void ocoms_graph_edge_construct(ocoms_graph_edge_t *edge);
static void ocoms_graph_edge_destruct(ocoms_graph_edge_t *edge);

OBJ_CLASS_INSTANCE(
    ocoms_graph_edge_t,
    ocoms_list_item_t,
    ocoms_graph_edge_construct,
    ocoms_graph_edge_destruct
);

static void ocoms_graph_construct(ocoms_graph_t *graph);
static void ocoms_graph_destruct(ocoms_graph_t *graph);

OBJ_CLASS_INSTANCE(
    ocoms_graph_t,
    ocoms_object_t,
    ocoms_graph_construct,
    ocoms_graph_destruct
);

static void ocoms_adjacency_list_construct(ocoms_adjacency_list_t *aj_list);
static void ocoms_adjacency_list_destruct(ocoms_adjacency_list_t *aj_list);

OBJ_CLASS_INSTANCE(
    ocoms_adjacency_list_t,
    ocoms_list_item_t,
    ocoms_adjacency_list_construct,
    ocoms_adjacency_list_destruct
);
...
/*
 *
 *      ocoms_graph_vertex_t interface
 *
 */
...
static void ocoms_graph_construct(ocoms_graph_t *graph)
{
    graph->adjacency_list = OBJ_NEW(ocoms_list_t);
...
static void ocoms_graph_destruct(ocoms_graph_t *graph)
{
    ocoms_adjacency_list_t *aj_list;

    while (false == ocoms_list_is_empty(graph->adjacency_list)) {
        aj_list = (ocoms_adjacency_list_t *)ocoms_list_remove_first(graph->adjacency_list);
        OBJ_RELEASE(aj_list);
    }
...
/*
 *
 *     ocoms_adjacency_list  interface
 *
 */
...
{
    aj_list->vertex = NULL;
    aj_list->edges = OBJ_NEW(ocoms_list_t);
}

static void ocoms_adjacency_list_destruct(ocoms_adjacency_list_t *aj_list)
{
   ocoms_graph_edge_t *edge;

   aj_list->vertex = NULL;
   while (false == ocoms_list_is_empty(aj_list->edges)) {
       edge = (ocoms_graph_edge_t *)ocoms_list_remove_first(aj_list->edges);
       OBJ_RELEASE(edge);
   }
 ...
 * @param vertex
 */
static void delete_all_edges_conceded_to_vertex(ocoms_graph_t *graph, ocoms_graph_vertex_t *vertex)
{
    ocoms_adjacency_list_t *aj_list;
    ocoms_list_item_t *aj_list_item;
    ocoms_graph_edge_t *edge;
    ocoms_list_item_t *edge_item;

    /**
     * for all the adjacency list in the graph
     */
    for (aj_list_item = ocoms_list_get_first(graph->adjacency_list);
         aj_list_item != ocoms_list_get_end(graph->adjacency_list);
         aj_list_item  = ocoms_list_get_next(aj_list_item)) {
        aj_list = (ocoms_adjacency_list_t *) aj_list_item;
        /**
         * for all the edges in the adjacency list
         */
        edge_item = ocoms_list_get_first(aj_list->edges);
        while (edge_item != ocoms_list_get_end(aj_list->edges)) {
            edge = (ocoms_graph_edge_t *)edge_item;
            edge_item  = ocoms_list_get_next(edge_item);
            ...
            if (edge->end == vertex) {
                /* Delete this edge  */
                ocoms_list_remove_item(edge->in_adj_list->edges, (ocoms_list_item_t*)edge);
                /* distract this edge */
                OBJ_RELEASE(edge);
            }
        }
 ...
 */
void ocoms_graph_add_vertex(ocoms_graph_t *graph, ocoms_graph_vertex_t *vertex)
{
   ocoms_adjacency_list_t *aj_list;
    ...
    */
   for (item = ocoms_list_get_first(graph->adjacency_list);
        item != ocoms_list_get_end(graph->adjacency_list);
        item  = ocoms_list_get_next(item)) {
       aj_list = (ocoms_adjacency_list_t *) item;
   ...
   aj_list = OBJ_NEW(ocoms_adjacency_list_t);
   ...
   ocoms_list_append(graph->adjacency_list, (ocoms_list_item_t*)aj_list);
 ...
 */
int ocoms_graph_add_edge(ocoms_graph_t *graph, ocoms_graph_edge_t *edge) 
{
    ocoms_adjacency_list_t *aj_list, *start_aj_list= NULL, *end_aj_list;
    ocoms_list_item_t *item;
    ...
    ocoms_list_append(start_aj_list->edges, (ocoms_list_item_t*)edge);
 ...
 */
void ocoms_graph_remove_edge (ocoms_graph_t *graph, ocoms_graph_edge_t *edge)
{
...
void ocoms_graph_remove_vertex(ocoms_graph_t *graph, ocoms_graph_vertex_t *vertex)
{
    ocoms_adjacency_list_t *adj_list;
    ocoms_graph_edge_t *edge;
    ...
    while (false == ocoms_list_is_empty(adj_list->edges)) {
        edge = (ocoms_graph_edge_t *)ocoms_list_remove_first(adj_list->edges);
        OBJ_RELEASE(edge);
    }
    /**
     ...
     * destruct it.
     */
    ocoms_list_remove_item(graph->adjacency_list, (ocoms_list_item_t*)adj_list);
 ...
 *         connected.
 */
uint32_t ocoms_graph_adjacent(ocoms_graph_t *graph, ocoms_graph_vertex_t *vertex1, ocoms_graph_vertex_t *vertex2)
{
    ocoms_adjacency_list_t *adj_list;
    ocoms_list_item_t *item;
    ocoms_graph_edge_t *edge;
    ...
    if (graph != vertex1->in_graph) {
        OCOMS_OUTPUT((0,"ocoms_graph_adjacent 1 Vertex1 %p not in the graph %p\n",(void *)vertex1,(void *)graph));
    ...
    if (graph != vertex2->in_graph) {
        OCOMS_OUTPUT((0,"ocoms_graph_adjacent 2 Vertex2 %p not in the graph %p\n",(void *)vertex2,(void *)graph));
        return DISTANCE_INFINITY;
    }
    /**
     ...
     * vertex.
     */
    adj_list = (ocoms_adjacency_list_t *) vertex1->in_adj_list;
    for (item = ocoms_list_get_first(adj_list->edges);
         item != ocoms_list_get_end(adj_list->edges);
         item  = ocoms_list_get_next(item)) {
        edge = (ocoms_graph_edge_t *)item;
        if (edge->end == vertex2) {
 ...
 * @return int
 */
int ocoms_graph_get_order(ocoms_graph_t *graph)
{
 ...
 * @return int
 */
int ocoms_graph_get_size(ocoms_graph_t *graph)
{
 ...
 */
ocoms_graph_vertex_t *ocoms_graph_find_vertex(ocoms_graph_t *graph, void *vertex_data)
{
    ocoms_adjacency_list_t *aj_list;
    ocoms_list_item_t *item;
 ...
 */
int ocoms_graph_get_graph_vertices(ocoms_graph_t *graph, ocoms_pointer_array_t *vertices_list)
{
    ocoms_adjacency_list_t *aj_list;
    ocoms_list_item_t *item;
    int i;
    ...
    /* Run on all the vertices of the graph */
    for (item = ocoms_list_get_first(graph->adjacency_list), i = 0;
         item != ocoms_list_get_end(graph->adjacency_list);
         item  = ocoms_list_get_next(item), i++) {
        aj_list = (ocoms_adjacency_list_t *) item;
        /* Add the vertex to the vertices array */
        ocoms_pointer_array_add(vertices_list,(void *)aj_list->vertex);
    }
 ...
 */
int ocoms_graph_get_adjacent_vertices(ocoms_graph_t *graph, ocoms_graph_vertex_t *vertex, ocoms_value_array_t *adjacents)
{
    ocoms_adjacency_list_t *adj_list;
    ocoms_graph_edge_t *edge;    
     ...
     */
    adj_list = (ocoms_adjacency_list_t *) vertex->in_adj_list;
    /* find the number of adjcents of this vertex */
    adjacents_number = ocoms_list_get_size(adj_list->edges);
    /* Run on all the edges from this vertex */
    for (item = ocoms_list_get_first(adj_list->edges), i = 0;
         item != ocoms_list_get_end(adj_list->edges);
         item  = ocoms_list_get_next(item), i++) {
        edge = (ocoms_graph_edge_t *)item;
        ...
        ocoms_value_array_append_item(adjacents, &distance_from);
    }
...
uint32_t ocoms_graph_spf(ocoms_graph_t *graph, ocoms_graph_vertex_t *vertex1, ocoms_graph_vertex_t *vertex2)
{
    ocoms_value_array_t *distance_array;
     ...
     */
    if (graph != vertex1->in_graph) {
        OCOMS_OUTPUT((0,"ocoms_graph_spf 1 Vertex1 %p not in the graph %p\n",(void *)vertex1,(void *)graph));
        return DISTANCE_INFINITY;
    }
    /**
     * Verify that the second vertex belongs to the graph.
     */
    if (graph != vertex2->in_graph) {
        OCOMS_OUTPUT((0,"ocoms_graph_spf 2 Vertex2 %p not in the graph %p\n",(void *)vertex2,(void *)graph));
        return DISTANCE_INFINITY;
    }
    /**
     * Run Dijkstra algorithm on the graph from the start vertex.
     */
    distance_array = OBJ_NEW(ocoms_value_array_t);
    ocoms_value_array_init(distance_array, sizeof(vertex_distance_from_t));
    ocoms_value_array_reserve(distance_array,50);
    items_in_distance_array = ocoms_graph_dijkstra(graph, vertex1, distance_array);
    /**
     * find the end vertex in the distance array that Dijkstra
     * algorithm returned.
     */
    for (i = 0; i < items_in_distance_array; i++) {
        vertex_distance = ocoms_value_array_get_item(distance_array, i);
 ...
 */
uint32_t ocoms_graph_dijkstra(ocoms_graph_t *graph, ocoms_graph_vertex_t *vertex, ocoms_value_array_t *distance_array)
{
    int graph_order;
    vertex_distance_from_t *Q, *q_start, *current_vertex;
    ocoms_list_item_t *adj_list_item;
    ocoms_adjacency_list_t *adj_list;
    ...
    graph_order = ocoms_graph_get_order(graph);
    Q = (vertex_distance_from_t *)malloc(graph_order * sizeof(vertex_distance_from_t));
    /* assign a pointer to the start of the queue */
    q_start = Q;
    /* run on all the vertices of the graph */
    for (adj_list_item = ocoms_list_get_first(graph->adjacency_list), i=0;
         adj_list_item != ocoms_list_get_end(graph->adjacency_list);
         adj_list_item  = ocoms_list_get_next(adj_list_item), i++) {
        adj_list = (ocoms_adjacency_list_t *)adj_list_item;
    ...
    while (number_of_items_in_q > 0) {
        ...
        for (i = 0; i < number_of_items_in_q; i++) {
            weight = ocoms_graph_adjacent(graph, current_vertex->vertex, q_start[i].vertex);
            /**
    ...
    for (i = 0; i < graph_order-1; i++) {
        ocoms_value_array_append_item(distance_array, (void *)&(Q[i+1]));
    }
 ...
 */
void ocoms_graph_duplicate(ocoms_graph_t **dest, ocoms_graph_t *src)
{
    ocoms_adjacency_list_t *aj_list;
    ocoms_list_item_t *aj_list_item, *edg_item;
    ocoms_graph_vertex_t *vertex;
    ocoms_graph_edge_t *edge, *new_edge;

    /* construct a new graph */
    *dest = OBJ_NEW(ocoms_graph_t);
    /* Run on all the vertices of the src graph */
    for (aj_list_item = ocoms_list_get_first(src->adjacency_list);
         aj_list_item != ocoms_list_get_end(src->adjacency_list);
         aj_list_item  = ocoms_list_get_next(aj_list_item)) {
        aj_list = (ocoms_adjacency_list_t *) aj_list_item;
        /* for each vertex in the src graph, construct a new vertex */
        vertex = OBJ_NEW(ocoms_graph_vertex_t);
        ...
        ocoms_graph_add_vertex(*dest, vertex);
    }
    /**
    ...
    for (aj_list_item = ocoms_list_get_first(src->adjacency_list);
        ...
        for (edg_item = ocoms_list_get_first(aj_list->edges);
             edg_item != ocoms_list_get_end(aj_list->edges);
             edg_item = ocoms_list_get_next(edg_item)) {
            edge = (ocoms_graph_edge_t *)edg_item;
            /* construct new edge for the new graph */
            new_edge = OBJ_NEW(ocoms_graph_edge_t);
            ...
            ocoms_graph_add_edge(*dest, new_edge);
        }
    }
}
 ...
 * @param graph
 */
void ocoms_graph_print(ocoms_graph_t *graph)
{
    ocoms_adjacency_list_t *aj_list;
    ocoms_list_item_t *aj_list_item;
    ocoms_graph_edge_t *edge;
    ...
    for (aj_list_item = ocoms_list_get_first(graph->adjacency_list);
        ...
        /* print vertex */
        ocoms_output(0, "V(%s) Connections:",tmp_str1);
        /* run on all the edges of the vertex */
        for (edge_item = ocoms_list_get_first(aj_list->edges);
             edge_item != ocoms_list_get_end(aj_list->edges);
             edge_item  = ocoms_list_get_next(edge_item)) {
            edge = (ocoms_graph_edge_t *)edge_item;
            ...
            /* print the edge */
            ocoms_output(0, "    E(%s -> %d -> %s)",tmp_str1, edge->weight, tmp_str2);
            if (need_free2) {
                free(tmp_str2);
            }
        }
