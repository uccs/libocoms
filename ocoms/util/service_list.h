 ...
 * @file 
 *
 * The service_list_t interface is used to provide a generic
 ...
 * when an service_list_t is destroyed, all of the service_list_item_t
 * objects that it contains are orphaned -- they are \em not
 * destroyed.
 *
 * The general idea is that service_list_item_t objects can be put on an
 * service_list_t.  Hence, you create a new type that derives from
 * service_list_item_t; this new type can then be used with service_list_t
 * containers.
 *
 * NOTE: service_list_item_t instances can only be on \em one list at a
 * time.  Specifically, if you add an service_list_item_t to one list,
 ...
 * threads may [erroneously] modify an service_list_item_t concurrently.
 *
 ...
 * \internal
 * 
 * Struct of an service_list_item_t
 */
struct service_list_item_t
{
    service_object_t super;
    /**< Generic parent class for all Open MPI objects */
    volatile struct service_list_item_t *service_list_next;
    /**< Pointer to next list item */
    volatile struct service_list_item_t *service_list_prev;
    ...
    volatile struct service_list_t* service_list_item_belong_to;
#endif
};
/**
 * Base type for items that are put in a list (service_list_t) containers.
 */
typedef struct service_list_item_t service_list_item_t;
 ...
 */
#define service_list_get_next(item) \
    ((item) ? ((service_list_item_t*) ((service_list_item_t*)(item))->service_list_next) : NULL)
 ...
 * @returns The next item in the list
 */
#define service_list_get_prev(item) \
    ((item) ? ((service_list_item_t*) ((service_list_item_t*)(item))->service_list_prev) : NULL)


/**
 * \internal
 *
 * Struct of an service_list_t
 */
struct service_list_t
{
    service_object_t       super;
    /**< Generic parent class for all Open MPI objects */
    service_list_item_t    service_list_sentinel;
    /**< Head and tail item of the list */
    volatile size_t     service_list_length;
    /**< Quick reference to the number of items in the list */
};
/**
 * List container type.
 */
typedef struct service_list_t service_list_t;
 ...
 */
static inline bool service_list_is_empty(service_list_t* list)
{
    return (list->service_list_sentinel.service_list_next == 
        &(list->service_list_sentinel) ? true : false);
}
 ...
 * service_list_get_end() to ensure that the list is not empty.
 *
 ...
 * should be compared against the returned value from
 * service_list_get_begin() to ensure that the list is not empty.
 *
 * This is an inlined function in compilers that support inlining, so
 * it's usually a cheap operation.
 */
 ...
 * it's usually a cheap operation.
 */
static inline service_list_item_t* service_list_get_begin(service_list_t* list)
{
    return &(list->service_list_sentinel);
}
 ...
 * it's usually a cheap operation.
 */
static inline service_list_item_t* service_list_get_end(service_list_t* list)
{
    return &(list->service_list_sentinel);
}
 ...
 * the future, calling \c service_list_splice or \c service_list_join may
 * result in this function recomputing the list size, which would be
 * an O(N) operation.  If \c service_list_splice or \c service_list_join is
...
static inline size_t service_list_get_size(service_list_t* list)
{
    ...
    for (item = service_list_get_first(list) ;
         item != service_list_get_end(list) ;
         item = service_list_get_next(item)) {
        check_len++;
    }

    if (check_len != list->service_list_length) {
        fprintf(stderr," Error :: service_list_get_size - service_list_length does not match actual list length\n");
        fflush(stderr);
        abort();
    }
#endif

    return list->service_list_length;
}
  ...
  (service_list_t *list, service_list_item_t *item)
{
    ...
    for (item_ptr = service_list_get_first(list);
            item_ptr != service_list_get_end(list);
            item_ptr = (service_list_item_t *)(item_ptr->service_list_next)) {
        if (item_ptr == (service_list_item_t *) item) {
            found = true;
            break;
        }
    }
    if (!found) {
        fprintf(stderr," Warning :: service_list_remove_item - the item %p is not on the list %p \n",(void*) item, (void*) list);
        fflush(stderr);
    ...
    item->service_list_prev->service_list_next=item->service_list_next;

    /* reset previous pointer of next element */
    item->service_list_next->service_list_prev=item->service_list_prev;
    ...
    return (service_list_item_t *)item->service_list_prev;
}
 ...
 * The service_list_item_t is not OBJ_RETAIN()'ed; it is assumed that
                                     ...
                                     )
{
  ...
  item->service_list_prev = sentinel->service_list_prev;

  /* reset previous pointer on current last element */
  sentinel->service_list_prev->service_list_next = item;

  /* reset new element's next pointer */
  item->service_list_next = sentinel;

  /* reset the list's tail element previous pointer */
  sentinel->service_list_prev = item;
 ...
 * list.  The service_list_item_t is not OBJ_RETAIN()'ed; it is assumed
 * that "ownership" of the item is passed from the caller to the list.
 *
 * This is an inlined function in compilers that support inlining, so
 * it's usually a cheap operation.
 */
static inline void service_list_prepend(service_list_t *list, 
                                     service_list_item_t *item) 
{
    service_list_item_t* sentinel = &(list->service_list_sentinel);
#if OCOMS_ENABLE_DEBUG
  ...
  item->service_list_next = sentinel->service_list_next;
  
  /* reset item's previous pointer */
  item->service_list_prev = sentinel;
  
  /* reset previous first element's previous poiner */
  sentinel->service_list_next->service_list_prev = item;
  
  /* reset head's next pointer */
  sentinel->service_list_next = item;
...
static inline service_list_item_t *service_list_remove_first(service_list_t *list)
{
  ...
  */
  ...
  item = list->service_list_sentinel.service_list_next;
  ...
  item->service_list_next=(service_list_item_t *)NULL;
  ...
  return (service_list_item_t *) item;
}
 ...
 * it's usually a cheap operation.
 */
static inline service_list_item_t *service_list_remove_last(service_list_t *list)
{
  ...
  */
  ...
  /* get item */
  item = list->service_list_sentinel.service_list_prev;
                                        ...
                                        service_list_item_t *item)
{
    ...
    item->service_list_next = pos;
    item->service_list_prev = pos->service_list_prev;

    /* splice into the list */
    pos->service_list_prev->service_list_next = item;
    pos->service_list_prev = item;
  ...
  OCOMS_DECLSPEC bool service_list_insert(service_list_t *list, service_list_item_t *item, 
     ...
     * but valid after the call.  All pointers to \c service_list_item_t
    ...
    /**
     * Comparison function for service_list_sort(), below.
     *
     * @param a Pointer to a pointer to an service_list_item_t.
     * Explanation below.
     * @param b Pointer to a pointer to an service_list_item_t.
     * Explanation below.
     ...
     * service_list_sort().  It is important to understand what
     * service_list_sort() does before invoking qsort, so go read that
     * documentation first.
     *
     ...
     *
     * \verb
     * static int compare(service_list_item_t **a, service_list_item_t **b)
     * {
     ...
     * \endverb
     */
    typedef int (*service_list_item_compare_fn_t)(service_list_item_t **a,
                                               service_list_item_t **b);
     ...
     *    (service_list_item_t*)'s in an array
     ...
     * a double de-reference to get to the actual service_list_item_t (or
