...
#ifndef OCOMS_MCA_BASE_VAR_H
#define OCOMS_MCA_BASE_VAR_H
...
#include "ocoms/mca/base/mca_base_var_group.h"
...
typedef enum {
    ...
    /** Setting the READONLY flag means that the ocoms_mca_base_var_set()
...
typedef enum {
    ...
    /** Variable can be set with ocoms_mca_base_var_set() */
    ...
    /** Variable should be deregistered when the group is deregistered
        ...
        ocoms_mca_base_component_var_register(), but can also be set
        manually when you register a variable with
        ocoms_mca_base_var_register().  Analogous to the
        MCA_BASE_PVAR_FLAG_IWG. */
    MCA_BASE_VAR_FLAG_DWG          = 0x0040
} ocoms_mca_base_var_flag_t;
...
typedef union {
    ...
    /** double value */
    double lfval;
} ocoms_mca_base_var_storage_t;


/**
 * Entry for holding information about an MCA variable.
 */
struct ocoms_mca_base_var_t {
    /** Allow this to be an OCOMS OBJ */
    ocoms_object_t super;

    /** Variable index. This will remain constant until ocoms_mca_base_var_finalize()
        is called. */
    int mbv_index;
    /** Group index. This will remain constant until ocoms_mca_base_var_finalize()
        is called. This variable will be deregistered if the associated group
        is deregistered with ocoms_mca_base_var_group_deregister() */
    int mbv_group_index;

    /** Info level of this variable */
    ocoms_mca_base_var_info_lvl_t mbv_info_lvl;

    /** Enum indicating the type of the variable (integer, string, boolean) */
     ocoms_mca_base_var_type_t mbv_type;
    ...
    ocoms_value_array_t mbv_synonyms;

    /** Variable flags */
    ocoms_mca_base_var_flag_t  mbv_flags;

    /** Variable scope */
    ocoms_mca_base_var_scope_t  mbv_scope;

    /** Source of the current value */
    ocoms_mca_base_var_source_t mbv_source;
    ...
    ocoms_mca_base_var_enum_t *mbv_enumerator;

    /** Bind value for this variable (0 - none) */
    int mbv_bind;

    /** Storage for this variable */
    ocoms_mca_base_var_storage_t *mbv_storage;
};
/**
 * Convenience typedef.
 */
typedef struct ocoms_mca_base_var_t ocoms_mca_base_var_t;
...
/**
 * Object declarayion for ocoms_mca_base_var_t
 */
OCOMS_DECLSPEC OBJ_CLASS_DECLARATION(mca_base_var_t);
 ...
 * invoked internally (by ocoms_mca_base_open()) and is only documented
 * here for completeness.
 */
OCOMS_DECLSPEC int ocoms_mca_base_var_init(void);
 ...
 * @retval OCOMS_ERR_OUT_OF_RESOURCE Upon failure to allocate memory.
 * @retval OCOMS_ERROR Upon failure to register the variable.
 *
 ...
 * ocoms_mca_base_var_enum_create(), and describes the valid values of an
 ...
 * ocoms_mca_base_var_deregister(), ocoms_mca_base_var_group_deregister(), or
 * ocoms_mca_base_var_finalize().  {enumerator} should be NULL for
 ...
 * be set using ocoms_mca_base_var_set_value() (i.e., the MPI_T interface).
 *
 ...
 * the description of ocoms_mca_base_var_scope_t for information about the
 ...
 * successfully.  
 */
OCOMS_DECLSPEC int ocoms_mca_base_var_register (const char *project_name, const char *framework_name,
                                         const char *component_name, const char *variable_name,
                                         const char *description, ocoms_mca_base_var_type_t type,
                                         ocoms_mca_base_var_enum_t *enumerator, int bind, ocoms_mca_base_var_flag_t flags,
 ...
 * component.
 *
 * While quite similar to ocoms_mca_base_var_register(), there is one key
 ...
 * its framework.
 */
OCOMS_DECLSPEC int ocoms_mca_base_component_var_register (const ocoms_mca_base_component_t *component,
 ...
 * function is equivalent to ocoms_mca_base_var_register with component_name = "base" and
 * with the MCA_BASE_VAR_FLAG_DWG set. See ocoms_mca_base_var_register().
 */
OCOMS_DECLSPEC int ocoms_mca_base_framework_var_register (const ocoms_mca_base_framework_t *framework,
 ...
 * @returns OCOMS_ERR_BAD_VAR If synonym_for does not reference a valid
 * variable.
 * @returns OCOMS_ERR_OUT_OF_RESOURCE If memory could not be allocated.
 * @returns OCOMS_ERROR For all other errors.
 * 
 ...
 * ocoms_mca_base_var_*() API).  If the user sets values for both MCA
 * variable names "B" and "C" (and does *not* set a value for
 * "A"), it is undefined as to which value will be used.
 */
OCOMS_DECLSPEC int ocoms_mca_base_var_register_synonym (int synonym_for, const char *project_name,
 ...
 *
 * @param index Index returned from ocoms_mca_base_var_register() or
 * ocoms_mca_base_var_register_synonym().
 *
 ...
 * ocoms_mca_base_var_finalize() is called.
 *
 * If an enumerator is associated with this variable it will be dereferenced.
 */
OCOMS_DECLSPEC int ocoms_mca_base_var_deregister(int index);
 ...
 *
 * @return OCOMS_ERROR Upon failure.  The contents of value are
 * undefined.
 * @return OCOMS_SUCCESS Upon success. value (if not NULL) will be filled
 ...
 * the ocoms_mca_base_var_* interface so the source may be incorrect.
 */
OCOMS_DECLSPEC int ocoms_mca_base_var_get_value (int index, const void *value,
 ...
 * @retval OCOMS_ERR_PERM If the variable is not settable.
 * @retval OCOMS_ERR_BAD_PARAM If the variable does not exist or has
 * been deregistered.
 * @retval OCOMS_ERROR On other error.
 *
 ...
 * settable.
 */
OCOMS_DECLSPEC int ocoms_mca_base_var_set_value (int index, const void *value, size_t size,
                                          ocoms_mca_base_var_source_t source,
                                          const char *source_file);
 ...
 */
OCOMS_DECLSPEC int ocoms_mca_base_var_env_name(const char *param_name,
                                        char **env_name);
 ...
 * ocoms_mca_base_var_get() and ocoms_mca_base_var_get_value().
 */
OCOMS_DECLSPEC int ocoms_mca_base_var_find (const char *project_name,
 ...
 *
 * See ocoms_mca_base_var_find().
 */
OCOMS_DECLSPEC int ocoms_mca_base_var_find_by_name (const char *full_name, int *index);
 ...
 *
 * This function will print an ocoms_show_help() message and return
 * OCOMS_ERR_BAD_VAR if it finds that the two variables both have
 ...
 * not default values).
 *
 * Note that ocoms_show_help() allows itself to be hooked, so if this
 * happens after the aggregated orte_show_help() system is
 * initialized, the messages will be aggregated (w00t).
 *
 * @returns OCOMS_ERR_BAD_VAR if the two variables have sources that
 * are not MCA_BASE_VAR_SOURCE_DEFAULT.
 * @returns OCOMS_SUCCESS otherwise.
 */
OCOMS_DECLSPEC int ocoms_mca_base_var_check_exclusive (const char *project,
                                                const char *type_a,
 ...
 *
 * @returns OCOMS_SUCCESS If the flags are set successfully.
 * @returns OCOMS_ERR_BAD_PARAM If the variable is not registered.
 * @returns OCOMS_ERROR Otherwise
 */
OCOMS_DECLSPEC int ocoms_mca_base_var_set_flag(int index, ocoms_mca_base_var_flag_t flag,
                                        bool set);
 ...
 *
 * @retval OCOMS_SUCCESS Upon success.
 * @retval ocoms error code Upon failure.
 *
 * The returned pointer belongs to the MCA variable system. Do not
 * modify/free/retain the pointer.
 */
OCOMS_DECLSPEC int ocoms_mca_base_var_get (int index, const ocoms_mca_base_var_t **var);

/**
 * Obtain the number of variables that have been registered.
 *
 * @retval count on success
 * @return ocoms error code on error
 *
 * Note: This function does not return the number of valid MCA variables as
 * ocoms_mca_base_var_deregister() has no impact on the variable count. The count
 * returned is equal to the number of calls to ocoms_mca_base_var_register with
 * unique names. ie. two calls with the same name will not affect the count.
 */
OCOMS_DECLSPEC int ocoms_mca_base_var_get_count (void);
 ...
 *
 * @retval OCOMS_SUCCESS Upon success.
 * @retval OCOMS_ERROR Upon failure.
 *
 * This function is similar to ocoms_mca_base_var_dump() except that
 * its output is in terms of an argv-style array of key=value
 * strings, suitable for using in an environment.
 */
OCOMS_DECLSPEC int ocoms_mca_base_var_build_env(char ***env, int *num_env,
                                         bool internal);
 ...
 *
 * @returns OCOMS_SUCCESS This function never fails.
 *
 * This function shuts down the MCA variable repository and frees all
 * associated memory.  No other ocoms_mca_base_var*() functions can be
 ...
 */
OCOMS_DECLSPEC int ocoms_mca_base_var_finalize(void);
...
} ocoms_mca_base_var_dump_type_t;
 ...
 */
OCOMS_DECLSPEC int ocoms_mca_base_var_dump(int index, char ***out, ocoms_mca_base_var_dump_type_t output_type);

END_C_DECLS

#endif /* OCOMS_MCA_BASE_VAR_H */
