...
static ocoms_hash_table_t ocoms_mca_base_pvar_index_hash;
static ocoms_pointer_array_t registered_pvars;
static bool ocoms_mca_base_pvar_initialized = false;
static int pvar_count = 0;

#define min(a,b) ((a) < (b) ? (a) : (b))
#define max(a,b) ((a) > (b) ? (a) : (b))

static int ocoms_mca_base_pvar_get_internal (int index, ocoms_mca_base_pvar_t **pvar, bool invalidok);
...
int ocoms_mca_base_pvar_init (void)
{
    int ret = OCOMS_SUCCESS;

    if (!ocoms_mca_base_pvar_initialized) {
        ocoms_mca_base_pvar_initialized = true;

        OBJ_CONSTRUCT(&registered_pvars, ocoms_pointer_array_t);
        ocoms_pointer_array_init(&registered_pvars, 128, 2048, 128);

        OBJ_CONSTRUCT(&ocoms_mca_base_pvar_index_hash, ocoms_hash_table_t);
        ret = ocoms_hash_table_init (&ocoms_mca_base_pvar_index_hash, 1024);
        if (OCOMS_SUCCESS != ret) {
            ocoms_mca_base_pvar_initialized = false;
            OBJ_DESTRUCT(&registered_pvars);
            OBJ_DESTRUCT(&ocoms_mca_base_pvar_index_hash);
        }
    }
...
int ocoms_mca_base_pvar_find (const char *project, const char *framework, const char *component, const char *name)
{
    char *full_name;
    int ret, index;

    ret = ocoms_mca_base_var_generate_full_name4 (NULL, framework, component, name, &full_name);
    if (OCOMS_SUCCESS != ret) {
        return OCOMS_ERROR;
    }

    ret = ocoms_mca_base_pvar_find_by_name (full_name, &index);
    free (full_name);
...
int ocoms_mca_base_pvar_find_by_name (const char *full_name, int *index)
{
    void *tmp;
    int rc;

    rc = ocoms_hash_table_get_value_ptr (&ocoms_mca_base_pvar_index_hash, full_name, strlen (full_name),
                                        &tmp);
    if (OCOMS_SUCCESS != rc) {
        return rc;
    }
...
int ocoms_mca_base_pvar_finalize (void)
{
    int i;
    ...
    if (ocoms_mca_base_pvar_initialized)  {
        ...
        for (i = 0 ; i < pvar_count ; ++i) {
            ocoms_mca_base_pvar_t *pvar = ocoms_pointer_array_get_item (&registered_pvars, i);
            if (pvar) {
                OBJ_RELEASE(pvar);
            }
        }
...
int ocoms_mca_base_pvar_get_count (int *count)
{
    *count = pvar_count;
    return OCOMS_SUCCESS;
}

static int ocoms_mca_base_pvar_default_get_value (const ocoms_mca_base_pvar_t *pvar, void *value, void *obj_handle)
{
    /* not used */
    (void) obj_handle;
...
static int ocoms_mca_base_pvar_default_set_value (ocoms_mca_base_pvar_t *pvar, const void *value, void *obj_handle)
{
    /* not used */
    (void) obj_handle;
...
static int ocoms_mca_base_pvar_notify_ignore (ocoms_mca_base_pvar_t *pvar, ocoms_mca_base_pvar_event_t event, void *obj_handle, int *count)
{
...
int ocoms_mca_base_pvar_register (const char *project, const char *framework, const char *component, const char *name,
                            const char *description, ocoms_mca_base_var_info_lvl_t verbosity,
                            int var_class, ocoms_mca_base_var_type_t type, ocoms_mca_base_var_enum_t *enumerator,
                            int bind, ocoms_mca_base_pvar_flag_t flags, ocoms_mca_base_get_value_fn_t get_value,
                            ocoms_mca_base_set_value_fn_t set_value, ocoms_mca_base_notify_fn_t notify, void *ctx)
{
    int ret, group_index;
    ocoms_mca_base_pvar_t *pvar;
    ...
    assert (verbosity >= OCOMS_INFO_LVL_1 && verbosity <= OCOMS_INFO_LVL_9);

    /* check if this variable is already registered */
    ret = ocoms_mca_base_pvar_find (project, framework, component, name);
    if (OCOMS_SUCCESS <= ret) {
        ret = ocoms_mca_base_pvar_get_internal (ret, &pvar, true);
        if (OCOMS_SUCCESS != ret) {
        ...
        group_index = ocoms_mca_base_var_group_register (project, framework, component, NULL);
        if (-1 > group_index) {
            return group_index;
        }

        /* create a new parameter entry */
        pvar = OBJ_NEW(ocoms_mca_base_pvar_t);
        if (NULL == pvar) {
        ...
        do {
            ...
            ocoms_mca_base_var_generate_full_name4 (NULL, framework, component, name, &pvar->name);
            if (NULL == pvar->name) {
            ...
            ret = ocoms_mca_base_var_group_add_pvar (group_index, pvar_count);
            if (0 > ret) {
                break;
            }

            ret = ocoms_pointer_array_add (&registered_pvars, pvar);
            if (0 > ret) {
                break;
            }

            ocoms_hash_table_set_value_ptr (&ocoms_mca_base_pvar_index_hash, pvar->name, strlen (pvar->name),
    ...
    pvar->get_value = get_value ? get_value : ocoms_mca_base_pvar_default_get_value;
    pvar->notify = notify ? notify : ocoms_mca_base_pvar_notify_ignore;

    if (!(flags & MCA_BASE_PVAR_FLAG_READONLY)) {
        pvar->set_value = set_value ? set_value : ocoms_mca_base_pvar_default_set_value;
    }
...
int ocoms_mca_base_component_pvar_register (const ocoms_mca_base_component_t *component, const char *name,
                                      const char *description, ocoms_mca_base_var_info_lvl_t verbosity,
                                      int var_class, ocoms_mca_base_var_type_t type, ocoms_mca_base_var_enum_t *enumerator,
                                      int bind, ocoms_mca_base_pvar_flag_t flags, ocoms_mca_base_get_value_fn_t get_value,
                                      ocoms_mca_base_set_value_fn_t set_value, ocoms_mca_base_notify_fn_t notify, void *ctx)
{
    ...
    return ocoms_mca_base_pvar_register(NULL, component->mca_type_name, component->mca_component_name,
...
static int ocoms_mca_base_pvar_get_internal (int index, ocoms_mca_base_pvar_t **pvar, bool invalidok)
{
    ...
    *pvar = ocoms_pointer_array_get_item (&registered_pvars, index);
...
int ocoms_mca_base_pvar_get (int index, const ocoms_mca_base_pvar_t **pvar)
{
    return ocoms_mca_base_pvar_get_internal (index, (ocoms_mca_base_pvar_t **) pvar, false);
}

int ocoms_mca_base_pvar_mark_invalid (int index)
{
    ocoms_mca_base_pvar_t *pvar;
    int ret;

    ret = ocoms_mca_base_pvar_get_internal (index, &pvar, false);
    if (OCOMS_SUCCESS != ret) {
        return ret;
    }
...
int ocoms_mca_base_pvar_update_all_handles (int index, const void *obj)
{
    ...
    if (0 == ocoms_list_get_size (&pvar->bound_handles)) {
        /* nothing to do */
        return OCOMS_SUCCESS;
    }

    /* TODO -- probably need to add a handle/variable lock */
    OCOMS_LIST_FOREACH_SAFE(handle, next, &pvar->bound_handles, ocoms_mca_base_pvar_handle_t) {
        handle = (ocoms_mca_base_pvar_handle_t *)((char *) handle - offsetof (ocoms_mca_base_pvar_handle_t, list2));

        if (handle->obj_handle != obj) {
            continue;
        }

        (void) ocoms_mca_base_pvar_handle_update (handle);
    }

    return OCOMS_SUCCESS;
}

int ocoms_mca_base_pvar_handle_alloc (ocoms_mca_base_pvar_session_t *session, int index, void *obj_handle,
                                ocoms_mca_base_pvar_handle_t **handle, int *count)
{
    ocoms_mca_base_pvar_handle_t *pvar_handle = NULL;
    ...
    do {
        ...
        pvar_handle = OBJ_NEW(ocoms_mca_base_pvar_handle_t);
        if (NULL == pvar_handle) {
            ret = OCOMS_ERR_OUT_OF_RESOURCE;
            break;
        }
        ...
        *handle = pvar_handle;
        ...
        ret = ocoms_mca_base_pvar_notify (pvar_handle, MCA_BASE_PVAR_HANDLE_BIND, count);
        if (0 > ret) {
            ret = OCOMS_ERROR;
            break;
        }
        ...
        /* get the size of this datatype since read functions will expect an
           array of datatype not ocoms_mca_base_pvar_value_t's. */
        ...
        ocoms_list_append (&session->handles, &pvar_handle->super);
        ocoms_list_append (&pvar->bound_handles, &pvar_handle->list2);
    ...
    if (OCOMS_SUCCESS != ret && pvar_handle) {
...
int ocoms_mca_base_pvar_handle_update (ocoms_mca_base_pvar_handle_t *handle)
{
    int i, ret;
    void *tmp;
    ...
    if (ocoms_mca_base_pvar_is_sum (handle->pvar) || ocoms_mca_base_pvar_is_watermark (handle->pvar)) {
    ...
    } else if (!ocoms_mca_base_pvar_is_continuous (handle->pvar)) {
...
int ocoms_mca_base_pvar_handle_read_value (ocoms_mca_base_pvar_handle_t *handle, void *value)
{
    int ret;
    ...
    if (ocoms_mca_base_pvar_is_sum (handle->pvar) || ocoms_mca_base_pvar_is_watermark (handle->pvar) ||
        !ocoms_mca_base_pvar_handle_is_running (handle)) {
...
int ocoms_mca_base_pvar_handle_start (ocoms_mca_base_pvar_handle_t *handle)
{
    int ret;
    ...
    ret = ocoms_mca_base_pvar_notify (handle, MCA_BASE_PVAR_HANDLE_START, NULL);
    if (OCOMS_SUCCESS != ret) {
        return ret;
    }
    ...
    } else if (ocoms_mca_base_pvar_is_watermark (handle->pvar)) {
...
int ocoms_mca_base_pvar_handle_stop (ocoms_mca_base_pvar_handle_t *handle)
{
    int ret;
    ...
    (void) ocoms_mca_base_pvar_notify (handle, MCA_BASE_PVAR_HANDLE_STOP, NULL);
...
int ocoms_mca_base_pvar_handle_reset (ocoms_mca_base_pvar_handle_t *handle)
{
    ...
    } else if (ocoms_mca_base_pvar_handle_is_running (handle) && ocoms_mca_base_pvar_is_watermark (handle->pvar)) {
            /* watermarks should get set to the current value if runnning. */

        ret = handle->pvar->get_value (handle->pvar, handle->current_value, handle->obj_handle);
    } else if (ocoms_mca_base_pvar_is_readonly (handle->pvar)) {
        return OCOMS_ERR_PERM;
    }
...
int ocoms_mca_base_pvar_dump(int index, char ***out, ocoms_mca_base_var_dump_type_t output_type)
{
    ...
    ret = ocoms_mca_base_pvar_get (index, &pvar);
    if (OCOMS_SUCCESS != ret) {
        return ret;
    }

    ret = ocoms_mca_base_var_group_get_internal (pvar->group_index, &group, true);
    if (OCOMS_SUCCESS != ret) {
        return ret;
    }
    ...
    if (MCA_BASE_VAR_DUMP_PARSABLE == output_type) {
        ...
        *out = (char **) calloc (line_count + 1, sizeof (char *));
        ...
        asprintf(out[0] + line++, "%sread-only:%s", tmp, ocoms_mca_base_pvar_is_readonly(pvar) ? "true" : "false");
        asprintf(out[0] + line++, "%scontinuous:%s", tmp, ocoms_mca_base_pvar_is_continuous(pvar) ? "true" : "false");
        asprintf(out[0] + line++, "%satomic:%s", tmp, ocoms_mca_base_pvar_is_atomic(pvar) ? "true" : "false");
...
/* ocoms_mca_base_pvar_t class */
static void ocoms_mca_base_pvar_contructor (ocoms_mca_base_pvar_t *pvar)
{
    memset ((char *) pvar + sizeof (pvar->super), 0, sizeof (*pvar) - sizeof (pvar->super));
    OBJ_CONSTRUCT(&pvar->bound_handles, ocoms_list_t);
}

static void ocoms_mca_base_pvar_destructor (ocoms_mca_base_pvar_t *pvar)
{
    if (pvar->name) {
        free (pvar->name);
    }
...
OBJ_CLASS_INSTANCE(ocoms_mca_base_pvar_t, ocoms_object_t, ocoms_mca_base_pvar_contructor, ocoms_mca_base_pvar_destructor);

/* ocoms_mca_base_pvar_session_t class */
static void ompi_mpi_pvar_session_constructor (ocoms_mca_base_pvar_session_t *session)
{
    OBJ_CONSTRUCT(&session->handles, ocoms_list_t);
}

static void ompi_mpi_pvar_session_destructor (ocoms_mca_base_pvar_session_t *session)
{
    ocoms_mca_base_pvar_handle_t *handle, *next;
...
OBJ_CLASS_INSTANCE(ocoms_mca_base_pvar_session_t, ocoms_object_t, ompi_mpi_pvar_session_constructor,
                   ompi_mpi_pvar_session_destructor);

/* ocoms_mca_base_pvar_handle_t class */
static void ocoms_mca_base_pvar_handle_constructor (ocoms_mca_base_pvar_handle_t *handle)
{
    memset ((char *) handle + sizeof (handle->super), 0, sizeof (*handle) - sizeof (handle->super));

    OBJ_CONSTRUCT(&handle->list2, ocoms_list_item_t);
}

static void ocoms_mca_base_pvar_handle_destructor (ocoms_mca_base_pvar_handle_t *handle)
{
    if (handle->pvar) {
        (void) ocoms_mca_base_pvar_notify (handle, MCA_BASE_PVAR_HANDLE_UNBIND, NULL);
    }
...
OBJ_CLASS_INSTANCE(ocoms_mca_base_pvar_handle_t, ocoms_list_item_t, ocoms_mca_base_pvar_handle_constructor,
                   ocoms_mca_base_pvar_handle_destructor);
