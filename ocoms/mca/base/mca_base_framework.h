...
#if !defined(OCOMS_MCA_BASE_FRAMEWORK_H)
#define OCOMS_MCA_BASE_FRAMEWORK_H
 ...
 */
enum ocoms_mca_base_register_flag_t {
...
typedef enum ocoms_mca_base_register_flag_t ocoms_mca_base_register_flag_t;

enum ocoms_mca_base_open_flag_t {
    MCA_BASE_OPEN_DEFAULT = 0,
    /** Find components in ocoms_mca_base_components_find. Used by
     ocoms_mca_base_framework_open() when NOREGISTER is specified
     by the framework */
    MCA_BASE_OPEN_FIND_COMPONENTS = 1,
};

typedef enum ocoms_mca_base_open_flag_t ocoms_mca_base_open_flag_t;
 ...
 * @retval ocoms error code on failure
 *
 * This function registers all framework MCA parameters. This
 * function should not call ocoms_mca_base_framework_components_register().
 *
 * Frameworks are NOT required to provide this function. It
 * may be NULL.
 */
typedef int (*ocoms_mca_base_framework_register_params_fn_t) (ocoms_mca_base_register_flag_t flags);
 ...
 *
 * This function is invoked during ocoms_init() and during the
 ...
 * default behavior of ocoms_mca_base_framework_open() is to call
 * ocoms_mca_base_framework_components_open(). If a framework provides
 * an open function it will need to call ocoms_mca_base_framework_components_open()
 * if it needs to open any components.
 */
typedef int (*ocoms_mca_base_framework_open_fn_t) (ocoms_mca_base_open_flag_t flags);

/**
 * Shut down the MCA framework.
 *
 * @retval OCOMS_SUCCESS Always
 *
 * This function should shut downs everything in the MCA
 * framework, and is called during ocoms_finalize() and the
 ...
 * default behavior of ocoms_mca_base_framework_close() is to call
 * ocoms_mca_base_framework_components_close(). If a framework provide
 * a close function it will need to call ocoms_mca_base_framework_components_close()
 * if any components were opened.
 */
typedef int (*ocoms_mca_base_framework_close_fn_t) (void);
...
typedef enum {
     ...
     */
} ocoms_mca_base_framework_flags_t;

typedef struct ocoms_mca_base_framework_t {
    /** Project name for this component (ex "ocoms") */
    ...
    ocoms_mca_base_framework_register_params_fn_t  framework_register;
    /** Framework open function or NULL */
    ocoms_mca_base_framework_open_fn_t             framework_open;
    /** Framework close function or NULL */
    ocoms_mca_base_framework_close_fn_t            framework_close;
    /** Framework flags (future use) set to 0 */
    ocoms_mca_base_framework_flags_t               framework_flags;
    /** Framework open count */
    int                                      framework_refcnt;
    /** List of static components */
    const ocoms_mca_base_component_t             **framework_static_components;
    ...
    /** List of selected components (filled in by ocoms_mca_base_framework_register()
        or ocoms_mca_base_framework_open() */
    ocoms_list_t                              framework_components;
} ocoms_mca_base_framework_t;
 ...
 */
OCOMS_DECLSPEC int ocoms_mca_base_framework_register (ocoms_mca_base_framework_t *framework,
 ...
 */
OCOMS_DECLSPEC int ocoms_mca_base_framework_close (ocoms_mca_base_framework_t *framework);

/**
 * Macro to declare an MCA framework
 *
 * Example:
 *  MCA_BASE_FRAMEWORK_DECLARE(ocoms, foo, NULL, ocoms_foo_open, ocoms_foo_close, MCA_BASE_FRAMEWORK_FLAG_LAZY)
 */
#define MCA_BASE_FRAMEWORK_DECLARE(project, name, description, registerfn, openfn, closefn, static_components, flags) \
    ocoms_mca_base_framework_t project##_##name##_base_framework = {   \
...
#endif /* OCOMS_MCA_BASE_FRAMEWORK_H */
