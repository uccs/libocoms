...
#include "ocoms/datatype/ocoms_datatype_unpack.h"
#include "ocoms/datatype/ocoms_datatype_prototypes.h"

#if defined(CHECKSUM)
#define ocoms_unpack_general_function            ocoms_unpack_general_checksum
#define ocoms_unpack_homogeneous_contig_function ocoms_unpack_homogeneous_contig_checksum
#define ocoms_generic_simple_unpack_function     ocoms_generic_simple_unpack_checksum
#else
#define ocoms_unpack_general_function            ocoms_unpack_general
#define ocoms_unpack_homogeneous_contig_function ocoms_unpack_homogeneous_contig
#define ocoms_generic_simple_unpack_function     ocoms_generic_simple_unpack
 ...
 */
int32_t
ocoms_unpack_general_function( ocoms_convertor_t* pConvertor,
                              struct iovec* iov,
                              uint32_t* out_size,
                              size_t* max_data )
{
    ...
    const ocoms_convertor_master_t* master = pConvertor->master;
        ...
        while( 1 ) {
            ...
            while( description[pos_desc].elem.common.flags & OCOMS_DATATYPE_FLAG_DATA ) {
                ...
                rc = master->pFunctions[type]( pConvertor, count_desc,
                                               pInput, iCount, ocoms_datatype_basicDatatypes[type]->size,
 ...
 * taken in account directly in the ocoms_convertor_unpack function (in convertor.c) for
 * the homogeneous case.
 */
int32_t
ocoms_unpack_homogeneous_contig_function( ocoms_convertor_t* pConv,
                                         struct iovec* iov,
                                         uint32_t* out_size,
                                         size_t* max_data )
{
    const ocoms_datatype_t *pData = pConv->pDesc;
    ...
    DO_DEBUG( ocoms_output( 0, "unpack_homogeneous_contig( pBaseBuf %p, iov_count %d )\n",
    ...
    for( iov_count = 0; iov_count < (*out_size); iov_count++ ) {
        ...
        if( (OCOMS_PTRDIFF_TYPE)pData->size == extent ) {
            ...
            DO_DEBUG( ocoms_output( 0, "unpack_homogeneous_contig( user_memory %p, packed_buffer %p length %lu\n",
            ...
            DO_DEBUG( ocoms_output( 0, "1. unpack contig dest %p src %p length %lu\n",
                                   user_memory, packed_buffer, (unsigned long)remaining ); );
            ...
            if( length != 0 ) {
                ...
                if( length <= remaining ) {
                    OCOMS_DATATYPE_SAFEGUARD_POINTER( user_memory, length, pConv->pBaseBuf,
                                                pData, pConv->count );
                    DO_DEBUG( ocoms_output( 0, "2. unpack dest %p src %p length %lu\n",
            ...
            for( i = 0; pData->size <= remaining; i++ ) {
                OCOMS_DATATYPE_SAFEGUARD_POINTER( user_memory, pData->size, pConv->pBaseBuf,
                                            pData, pConv->count );
                DO_DEBUG( ocoms_output( 0, "3. unpack dest %p src %p length %lu\n",
            ...
            if( remaining != 0 ) {
                OCOMS_DATATYPE_SAFEGUARD_POINTER( user_memory, remaining, pConv->pBaseBuf,
                                            pData, pConv->count );
                DO_DEBUG( ocoms_output( 0, "4. unpack dest %p src %p length %lu\n",
                                       user_memory, packed_buffer, (unsigned long)remaining ); );
                MEMCPY_CSUM( user_memory, packed_buffer, remaining, pConv );
...
ocoms_unpack_partial_datatype( ocoms_convertor_t* pConvertor, dt_elem_desc_t* pElem,
                              ...
                              unsigned char** user_buffer )
{
    ...
    size_t data_length = ocoms_datatype_basicDatatypes[pElem->elem.common.type]->size;

    DO_DEBUG( ocoms_output( 0, "unpack partial data start %lu end %lu data_length %lu user %p\n"
 ...
 find_unused_byte:
    ...
    data_length = ocoms_datatype_basicDatatypes[pElem->elem.common.type]->size;
 ...
 */
int32_t
ocoms_generic_simple_unpack_function( ocoms_convertor_t* pConvertor,
                                     ...
                                     size_t* max_data )
{
    ...
    DO_DEBUG( ocoms_output( 0, "ocoms_convertor_generic_simple_unpack( %p, {%p, %lu}, %u )\n",
    ...
    DO_DEBUG( ocoms_output( 0, "unpack start pos_desc %d count_desc %d disp %ld\n"
    ...
    for( iov_count = 0; iov_count < (*out_size); iov_count++ ) {
        ...
        if( 0 != pConvertor->partial_length ) {
            size_t element_length = ocoms_datatype_basicDatatypes[pElem->elem.common.type]->size;
            ...
            ocoms_unpack_partial_datatype( pConvertor, pElem,
                                          packed_buffer,
        ...
        while( 1 ) {
            while( pElem->elem.common.flags & OCOMS_DATATYPE_FLAG_DATA ) {
                ...
                if( 0 != iov_len_local ) {
                     ...
                     */
                    assert( iov_len_local < ocoms_datatype_basicDatatypes[type]->size );
            ...
            if( OCOMS_DATATYPE_END_LOOP == pElem->elem.common.type ) { /* end of the current loop */
                DO_DEBUG( ocoms_output( 0, "unpack end_loop count %d stack_pos %d pos_desc %d disp %ld space %lu\n",
                ...
                user_memory_base = pConvertor->pBaseBuf + pStack->disp;
                UPDATE_INTERNAL_COUNTERS( description, pos_desc, pElem, count_desc );
                DO_DEBUG( ocoms_output( 0, "unpack new_loop count %d stack_pos %d pos_desc %d disp %ld space %lu\n",
                                       (int)pStack->count, pConvertor->stack_pos, pos_desc,
                                       (long)pStack->disp, (unsigned long)iov_len_local ); );
            }
 ...
 complete_conversion:
    ...
    *out_size = iov_count;
    ...
    DO_DEBUG( ocoms_output( 0, "unpack save stack stack_pos %d pos_desc %d count_desc %d disp %ld\n",
                           pConvertor->stack_pos, pStack->index, (int)pStack->count, (long)pStack->disp ); );
    return 0;
}
