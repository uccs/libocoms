...
#include "ocoms/datatype/ocoms_datatype_pack.h"
#include "ocoms/datatype/ocoms_datatype_prototypes.h"

#if defined(CHECKSUM)
#define ocoms_pack_homogeneous_contig_function           ocoms_pack_homogeneous_contig_checksum
#define ocoms_pack_homogeneous_contig_with_gaps_function ocoms_pack_homogeneous_contig_with_gaps_checksum
#define ocoms_generic_simple_pack_function               ocoms_generic_simple_pack_checksum
#else
#define ocoms_pack_homogeneous_contig_function           ocoms_pack_homogeneous_contig
#define ocoms_pack_homogeneous_contig_with_gaps_function ocoms_pack_homogeneous_contig_with_gaps
#define ocoms_generic_simple_pack_function               ocoms_generic_simple_pack
 ...
 */
int32_t
ocoms_pack_homogeneous_contig_function( ocoms_convertor_t* pConv,
                                       struct iovec* iov,
                                       uint32_t* out_size,
                                       size_t* max_data )
{
...
int32_t
ocoms_pack_homogeneous_contig_with_gaps_function( ocoms_convertor_t* pConv,
                                                 struct iovec* iov,
                                                 uint32_t* out_size,
                                                 size_t* max_data )
{
 ...
 */
int32_t
ocoms_generic_simple_pack_function( ocoms_convertor_t* pConvertor,
                                   ...
                                   size_t* max_data )
{
    ...
    DO_DEBUG( ocoms_output( 0, "pack start pos_desc %d count_desc %d disp %ld\n"
    ...
    for( iov_count = 0; iov_count < (*out_size); iov_count++ ) {
        ...
        while( 1 ) {
            ...
            if( OCOMS_DATATYPE_END_LOOP == pElem->elem.common.type ) { /* end of the current loop */
                ...
                source_base = pConvertor->pBaseBuf + pStack->disp;
                UPDATE_INTERNAL_COUNTERS( description, pos_desc, pElem, count_desc );
                DO_DEBUG( ocoms_output( 0, "pack new_loop count %d stack_pos %d pos_desc %d disp %ld space %lu\n",
    ...
    *out_size = iov_count;
    ...
    DO_DEBUG( ocoms_output( 0, "pack save stack stack_pos %d pos_desc %d count_desc %d disp %ld\n",
                           pConvertor->stack_pos, pStack->index, (int)pStack->count, (long)pStack->disp ); );
    return 0;
}
