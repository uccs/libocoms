...
#include "ocoms/primitives/types.h"
...
static inline void
ocoms_dt_swap_bytes(void *to_p, const void *from_p, const size_t size)
{
    size_t i;
...
static int32_t                                                                            \
copy_##TYPENAME##_heterogeneous(ocoms_convertor_t *pConvertor, uint32_t count,             \
...
{                                                                       \
    ...
    datatype_check( #TYPE, sizeof(TYPE), sizeof(TYPE), &count,          \
                   ...
                   to, to_length, to_extent);                           \
                                                                        \
    if ((pConvertor->remoteArch & OCOMS_ARCH_ISBIGENDIAN) !=             \
        (ocoms_local_arch & OCOMS_ARCH_ISBIGENDIAN)) {                    \
        for( i = 0; i < count; i++ ) {                                  \
            ocoms_dt_swap_bytes(to, from, sizeof(TYPE));                 \
            to += to_extent;                                            \
            from += from_extent;                                        \
        }                                                               \
...
{                                                                       \
    ...
    if ((pConvertor->remoteArch & OCOMS_ARCH_ISBIGENDIAN) !=             \
        ...
        for( i = 0; i < count; i++ ) {                                  \
            ...
            ocoms_dt_swap_bytes(to_1, from_1, sizeof(TYPE1));            \
            to_2 = (TYPE2*) (to_1 + 1); from_2 = (TYPE2*) (from_1 + 1); \
            ocoms_dt_swap_bytes(to_2, from_2, sizeof(TYPE2));            \
            to += to_extent;                                            \
            from += from_extent;                                        \
        }                                                               \
...
static int32_t
copy_cxx_bool_heterogeneous(ocoms_convertor_t *pConvertor, uint32_t count,
                            const char* from, uint32_t from_len, OCOMS_PTRDIFF_TYPE from_extent,
                            char* to, uint32_t to_length, OCOMS_PTRDIFF_TYPE to_extent,
                            OCOMS_PTRDIFF_TYPE *advance)
{
    ...
    if ((pConvertor->remoteArch & OCOMS_ARCH_BOOLISxx) !=
        (ocoms_local_arch & OCOMS_ARCH_BOOLISxx)) {
        switch (pConvertor->remoteArch & OCOMS_ARCH_BOOLISxx) {
        case OCOMS_ARCH_BOOLIS8:
            from_extent = 1;
            break;
        case OCOMS_ARCH_BOOLIS16:
            from_extent = 2;
            break;
        case OCOMS_ARCH_BOOLIS32:
            from_extent = 4;
            break;
        }
    }
    ...
    if ((to_extent != sizeof(bool) || from_extent != sizeof(bool)) ||
        ((pConvertor->remoteArch & OCOMS_ARCH_BOOLISxx) !=
         (ocoms_local_arch & OCOMS_ARCH_BOOLISxx))) {
        switch (pConvertor->remoteArch & OCOMS_ARCH_BOOLISxx) {
        case OCOMS_ARCH_BOOLIS8:
...
#else
/* #error No basic type for copy function for ocoms_datatype_float2 found */
...
#else
/* #error No basic type for copy function for ocoms_datatype_float4 found */
...
#else
/* #error No basic type for copy function for ocoms_datatype_float8 found */
...
#else
/* #error No basic type for copy function for ocoms_datatype_float12 found */
...
#else
/* #error No basic type for copy function for ocoms_datatype_float16 found */
...
conversion_fct_t ocoms_datatype_heterogeneous_copy_functions[OCOMS_DATATYPE_MAX_PREDEFINED] = {
