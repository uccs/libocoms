...
static inline void pack_predefined_data( ocoms_convertor_t* CONVERTOR,
                                         dt_elem_desc_t* ELEM,
                                         uint32_t* COUNT,
                                         ...
                                         size_t* SPACE )
{
    ...
    if( (OCOMS_PTRDIFF_TYPE)_copy_blength == _elem->extent ) {
        ...
        DO_DEBUG( ocoms_output( 0, "pack 1. memcpy( %p, %p, %lu ) => space %lu\n",
        ...
        *(DESTINATION) += _copy_blength;
        ...
        for( _i = 0; _i < _copy_count; _i++ ) {
            OCOMS_DATATYPE_SAFEGUARD_POINTER( _source, _copy_blength, (CONVERTOR)->pBaseBuf,
                                        (CONVERTOR)->pDesc, (CONVERTOR)->count );
            DO_DEBUG( ocoms_output( 0, "pack 2. memcpy( %p, %p, %lu ) => space %lu\n",
...
static inline void pack_contiguous_loop( ocoms_convertor_t* CONVERTOR,
                                         dt_elem_desc_t* ELEM,
                                         uint32_t* COUNT,
                                         unsigned char** SOURCE,
                                         unsigned char** DESTINATION,
                                         size_t* SPACE )
{
    ...
    for( _i = 0; _i < _copy_loops; _i++ ) {
        OCOMS_DATATYPE_SAFEGUARD_POINTER( _source, _end_loop->size, (CONVERTOR)->pBaseBuf,
                                    (CONVERTOR)->pDesc, (CONVERTOR)->count );
        DO_DEBUG( ocoms_output( 0, "pack 3. memcpy( %p, %p, %lu ) => space %lu\n",
