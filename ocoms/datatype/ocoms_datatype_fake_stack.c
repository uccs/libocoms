...
int ocoms_convertor_create_stack_with_pos_general( ocoms_convertor_t* pConvertor,
                                                  size_t starting_point,
                                                  const size_t* sizes );

static inline size_t
ocoms_convertor_compute_remote_size( const ocoms_datatype_t* pData, const size_t* sizes )
{
    uint32_t i;
    size_t length = 0;
                                                  ...
                                                  size_t starting_point, const size_t* sizes )
{
    ...
    /*ocoms_output( 0, "Data extent %d size %d count %d total_size %d starting_point %d\n",
    ...
    remote_size    = ocoms_convertor_compute_remote_size( pData, sizes );
    ...
    while( pos_desc < (int32_t)pConvertor->use_desc->used ) {
        ...
        while( pElems->elem.common.flags & OCOMS_DATATYPE_FLAG_DATA ) {
            ...
            const ocoms_datatype_t* basic_type = BASIC_DDT_FROM_ELEM( (*pElems) );
