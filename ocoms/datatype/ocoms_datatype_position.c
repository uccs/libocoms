...
static inline void position_predefined_data( ocoms_convertor_t* CONVERTOR,
                                             dt_elem_desc_t* ELEM,
                                             uint32_t* COUNT,
                                             ...
                                             size_t* SPACE )
{
    ...
    _copy_blength =  ocoms_datatype_basicDatatypes[_elem->common.type]->size;
...
static inline void position_contiguous_loop( ocoms_convertor_t* CONVERTOR,
                                             dt_elem_desc_t* ELEM,
                                             uint32_t* COUNT,
                                             unsigned char** POINTER,
                                             size_t* SPACE )
...
int ocoms_convertor_generic_simple_position( ocoms_convertor_t* pConvertor,
                                            size_t* position )
{
    ...
    DUMP( "ocoms_convertor_generic_simple_position( %p, &%ld )\n", (void*)pConvertor, (long)*position );
    ...
    if( iov_len_local > pConvertor->pDesc->size ) {
        ...
        DO_DEBUG( ocoms_output( 0, "position before %lu asked %lu data size %lu"
        ...
        DO_DEBUG( ocoms_output( 0, "after bConverted %lu remaining count %lu iov_len_local %lu\n",
    ...
    DO_DEBUG( ocoms_output( 0, "position start pos_desc %d count_desc %d disp %llx\n"
    ...
    while( 1 ) {
        if( OCOMS_DATATYPE_END_LOOP == pElem->elem.common.type ) { /* end of the current loop */
            DO_DEBUG( ocoms_output( 0, "position end_loop count %d stack_pos %d pos_desc %d disp %llx space %lu\n",
            ...
            DO_DEBUG( ocoms_output( 0, "position new_loop count %d stack_pos %d pos_desc %d disp %llx space %lu\n",
                                   (int)pStack->count, pConvertor->stack_pos, pos_desc,
                                   (unsigned long long)pStack->disp, (unsigned long)iov_len_local ); );
        }
        ...
        update_loop_description:  /* update the current state */
            ...
            DO_DEBUG( ocoms_output( 0, "position set loop count %d stack_pos %d pos_desc %d disp %llx space %lu\n",
                                   (int)pStack->count, pConvertor->stack_pos, pos_desc,
                                   (unsigned long long)pStack->disp, (unsigned long)iov_len_local ); );
            continue;
        }
 ...
 complete_loop:
    ...
    if( !(pConvertor->flags & CONVERTOR_COMPLETED) ) {
        ...
        DO_DEBUG( ocoms_output( 0, "position save stack stack_pos %d pos_desc %d count_desc %d disp %llx\n",
                               pConvertor->stack_pos, pStack->index, (int)pStack->count, (unsigned long long)pStack->disp ); );
        return 0;
    }
    return 1;
