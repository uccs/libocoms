...
static inline int ADD_ELEMENT( dt_elem_desc_t* pElemDesc __ocoms_attribute_unused__,
                               ddt_elem_desc_t* opt_elem,
...
static int32_t
ocoms_datatype_optimize_short( ocoms_datatype_t* pData,
                         int32_t count,
                         dt_type_desc_t* pTypeDesc )
{
    ...
    int32_t stack_pos = 0, last_type = OCOMS_DATATYPE_UINT1, last_length = 0;
    int32_t type = OCOMS_DATATYPE_LOOP, nbElems = 0, continuity;
    OCOMS_PTRDIFF_TYPE total_disp = 0, last_extent = 1, last_disp = 0;
    ...
    while( stack_pos >= 0 ) {
        ...
        if( OCOMS_DATATYPE_LOOP == pData->desc.desc[pos_desc].elem.common.type ) {
            ...
            continuity = ((last_disp + last_length * (OCOMS_PTRDIFF_TYPE)ocoms_datatype_basicDatatypes[last_type]->size)
            ...
            if( loop->common.flags & OCOMS_DATATYPE_FLAG_CONTIGUOUS ) {
                ...
                if( loop->extent == (OCOMS_PTRDIFF_TYPE)end_loop->size ) {
                    ...
                    if( !continuity ) {
                        if( 0 != last_length ) {
                            CREATE_ELEM( pElemDesc, last_type, OCOMS_DATATYPE_FLAG_BASIC,
                    ...
                    last_length = (last_length * ocoms_datatype_basicDatatypes[last_type]->size
                                   + loop->loops * end_loop->size);
                    last_type   = OCOMS_DATATYPE_UINT1;
                    last_extent = 1;
                } else {
                    int counter = loop->loops;
                    OCOMS_PTRDIFF_TYPE merged_disp = 0;
                    /* if the previous data is contiguous with this piece and it has a length not ZERO */
                    if( last_length != 0 ) {
                        if( continuity ) {
                            last_length *= ocoms_datatype_basicDatatypes[last_type]->size;
                    ...
                    if( counter <= 2 ) {
                        ...
                        while( counter > 0 ) {
                            CREATE_ELEM( pElemDesc, OCOMS_DATATYPE_UINT1, OCOMS_DATATYPE_FLAG_BASIC,
                ...
                if( 2 == loop->items ) { /* small loop */
                    if( (1 == elem->count)
                        && (elem->extent == (OCOMS_PTRDIFF_TYPE)ocoms_datatype_basicDatatypes[elem->common.type]->size) ) {
        ...
        while( pData->desc.desc[pos_desc].elem.common.flags & OCOMS_DATATYPE_FLAG_DATA ) {  /* keep doing it until we reach a non datatype element */
            ...
            if( (pData->desc.desc[pos_desc].elem.common.flags & OCOMS_DATATYPE_FLAG_CONTIGUOUS) && continuity &&
                (pData->desc.desc[pos_desc].elem.extent == (int32_t)ocoms_datatype_basicDatatypes[type]->size) ) {
                if( type == last_type ) {
                    ...
                    if( last_length == 0 ) {
                        ...
                        last_length = last_length * ocoms_datatype_basicDatatypes[last_type]->size +
                            pData->desc.desc[pos_desc].elem.count * ocoms_datatype_basicDatatypes[type]->size;
                        last_type = OCOMS_DATATYPE_UINT1;
                        last_extent = 1;
                    }
                }
...
int32_t ocoms_datatype_commit( ocoms_datatype_t * pData )
{
    ...
    (void)ocoms_datatype_optimize_short( pData, 1, &(pData->opt_desc) );
